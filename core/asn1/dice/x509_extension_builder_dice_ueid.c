...
int x509_extension_builder_dice_ueid_build_dynamic (const struct x509_extension_builder *builder,
	struct x509_extension *extension)
{
	const struct x509_extension_builder_dice_ueid *dice =
	...
	length = X509_EXTENSION_BUILDER_DICE_UEID_DATA_LENGTH (dice->ueid_length);
	buffer = platform_malloc (length);
	if (buffer == NULL) {
		return DICE_UEID_EXTENSION_NO_MEMORY;
	}

	status = x509_extension_builder_dice_ueid_create_extension (dice, buffer, length, extension);
	if (status != 0) {
	...
	struct x509_extension *extension)
{
	...
	return x509_extension_builder_dice_ueid_create_extension (dice, dice->ext_buffer,
	...
	const uint8_t *ueid, size_t length)
{
	...
	memset (builder, 0, sizeof (struct x509_extension_builder_dice_ueid));

	builder->base.build = x509_extension_builder_dice_ueid_build_dynamic;
	builder->base.free = x509_extension_builder_dice_ueid_free_dynamic;
	...
	uint8_t *ext_buffer, size_t buffer_length)
{
	...
	if (buffer_length < X509_EXTENSION_BUILDER_DICE_UEID_DATA_LENGTH (length)) {
		return DICE_UEID_EXTENSION_SMALL_EXT_BUFFER;
	}

	memset (builder, 0, sizeof (struct x509_extension_builder_dice_ueid));

	builder->base.build = x509_extension_builder_dice_ueid_build_static;
	builder->base.free = x509_extension_builder_dice_ueid_free_static;
