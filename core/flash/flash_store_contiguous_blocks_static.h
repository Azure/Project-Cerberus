 ...
 * @param hash_ptr Optional hash engine instance for integrity checking.  This should be null when
 * using FLASH_STORE_CONTIGUOUS_BLOCKS_NO_HASH_API_INIT.
 */
#define	flash_store_contiguous_blocks_static_init_fixed_storage(api, state_ptr, flash_addr, \
	block_count, flash_ptr, hash_ptr) { \
		.base = api, \
		.state = state_ptr, \
...
#define	flash_store_contiguous_blocks_static_init_fixed_storage_decreasing(api, state_ptr, \
	flash_addr, block_count, flash_ptr, hash_ptr) { \
		.base = api, \
		.state = state_ptr, \
		.base_addr = flash_addr, \
		.blocks = block_count, \
