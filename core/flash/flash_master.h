 ...
 */
enum {
	FLASH_MASTER_INVALID_ARGUMENT = FLASH_MASTER_ERROR (0),		/**< Input parameter is null or not valid. */
	FLASH_MASTER_NO_MEMORY = FLASH_MASTER_ERROR (1),			/**< Memory allocation failed. */
	FLASH_MASTER_XFER_FAILED = FLASH_MASTER_ERROR (2),			/**< The SPI transaction did not complete. */
	FLASH_MASTER_UNSUPPORTED_XFER = FLASH_MASTER_ERROR (3),		/**< The SPI transfer uses capabilities not supported by the driver. */
	FLASH_MASTER_HW_NOT_INIT = FLASH_MASTER_ERROR (4),			/**< The SPI hardware has not been initialized. */
	FLASH_MASTER_XFER_TIMEOUT = FLASH_MASTER_ERROR (5),			/**< The SPI transfer has timed out without completing. */
	FLASH_MASTER_NO_XFER_DATA = FLASH_MASTER_ERROR (6),			/**< No data buffer was provided for the transfer. */
	FLASH_MASTER_XFER_DMA_ERROR = FLASH_MASTER_ERROR (7),		/**< The was a DMA error while executing the transfer. */
};
