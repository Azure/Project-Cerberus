...
#include "host_processor_dual_full_bypass_reset_flash.h"
#include "host_state_manager.h"


static int host_processor_dual_full_bypass_reset_flash_soft_reset (struct host_processor *host,
	struct hash_engine *hash, struct rsa_engine *rsa)
{
	...
	return host_processor_filtered_update_verification (dual, hash, rsa, false, true, 0, true);
}
	...
	struct recovery_image_manager *recovery)
{
	...
	status = host_processor_dual_full_bypass_init (host, control, flash, state, filter, pfm,
		recovery);
	if (status != 0) {
		return status;
	}

	host->base.soft_reset = host_processor_dual_full_bypass_reset_flash_soft_reset;
	...
	struct recovery_image_manager *recovery, int pulse_width)
{
	...
	status = host_processor_dual_full_bypass_init_pulse_reset (host, control, flash, state, filter, pfm,
 ...
 */
void host_processor_dual_full_bypass_reset_flash_release (struct host_processor_filtered *host)
{
