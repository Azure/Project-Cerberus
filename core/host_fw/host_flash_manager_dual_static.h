...
#ifndef HOST_FLASH_MANAGER_DUAL_STATIC_H_
#define HOST_FLASH_MANAGER_DUAL_STATIC_H_
...
#define	HOST_FLASH_MANAGER_DUAL_API_INIT  { \
		...
		.validate_read_write_flash = host_flash_manager_dual_validate_read_write_flash, \
		.get_flash_read_write_regions = host_flash_manager_dual_get_flash_read_write_regions, \
		.free_read_write_regions = host_flash_manager_free_read_write_regions, \
		.config_spi_filter_flash_type = host_flash_manager_dual_config_spi_filter_flash_type, \
		.config_spi_filter_flash_devices = \
			host_flash_manager_dual_config_spi_filter_flash_devices, \
		.swap_flash_devices = host_flash_manager_dual_swap_flash_devices, \
		.initialize_flash_protection = host_flash_manager_dual_initialize_flash_protection, \
		.restore_flash_read_write_regions = \
			host_flash_manager_dual_restore_flash_read_write_regions, \
		.set_flash_for_rot_access = host_flash_manager_dual_set_flash_for_rot_access, \
		.set_flash_for_host_access = host_flash_manager_dual_set_flash_for_host_access, \
		.host_has_flash_access = host_flash_manager_dual_host_has_flash_access, \
		.reset_flash = host_flash_manager_dual_reset_flash, \
	}


/**
 * Initialize a static instance of the manager for dual host flash devices.
 *
 ...
 */
#define	host_flash_manager_dual_static_init(cs0_ptr, cs1_ptr, host_state_ptr, filter_ptr, \
	mfg_handler_ptr)	{ \
		.base = HOST_FLASH_MANAGER_DUAL_API_INIT, \
		.flash_cs0 = cs0_ptr, \
		.flash_cs1 = cs1_ptr, \
...
/**
 * Initialize a static instance of the manager for dual host flash devices.  The interfaces to the
 * flash devices may be uninitialized, but an initialization manager is provided to ensure they get
 ...
 */
#define	host_flash_manager_dual_static_init_with_managed_flash_initialization(cs0_ptr, cs1_ptr, \
...
#endif	/* HOST_FLASH_MANAGER_DUAL_STATIC_H_ */
