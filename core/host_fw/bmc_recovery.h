...
struct bmc_recovery_control {
	size_t max_wdt;					/**< The maximum number of watchdogs events to trigger recovery. */
	size_t min_wdt;					/**< The minimum number of watchdog events to trigger a rollback. */
...
struct bmc_recovery {
	...
	size_t num_wdt;							/**< A counter for the number of watchdog events. */
...
int bmc_recovery_init (struct bmc_recovery *recovery, struct host_irq_control *irq,
	struct host_processor *host, struct host_control *control, struct bmc_recovery_control *rec_ctrl);
