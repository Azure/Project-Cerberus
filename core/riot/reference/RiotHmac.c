...
void RIOT_HMAC_SHA256_Init (
	RIOT_HMAC_SHA256_CTX *ctx, const uint8_t *key, size_t keyLen)
{
	size_t cnt;
...
void RIOT_HMAC_SHA256_Update (
	RIOT_HMAC_SHA256_CTX *ctx, const uint8_t *data, size_t dataLen)
{
	RIOT_SHA256_Update (&ctx->hashCtx, data, dataLen);
}

void RIOT_HMAC_SHA256_Final (
	RIOT_HMAC_SHA256_CTX *ctx, uint8_t *digest)
{
