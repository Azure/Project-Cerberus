 ...
 */
enum {
	TPM_INVALID_ARGUMENT = TPM_ERROR (0),			/**< Input parameter is null or not valid. */
	TPM_NO_MEMORY = TPM_ERROR (1),					/**< Memory allocation failed. */
	TPM_STORAGE_WRITE_FAIL = TPM_ERROR (2),			/**< Failure when writing to persistent storage. */
	TPM_INVALID_STORAGE = TPM_ERROR (3),			/**< TPM storage contents invalid. */
	TPM_INVALID_LEN = TPM_ERROR (4),				/**< Buffer provided too small for requested contents. */
	TPM_OUT_OF_RANGE = TPM_ERROR (5),				/**< Parameter out of range. */
	TPM_INVALID_SEGMENT = TPM_ERROR (6),			/**< TPM storage segment not written to. */
	TPM_STORAGE_NOT_ALIGNED = TPM_ERROR (7),		/**< Address for the TPM is not aligned correctly. */
	TPM_INSUFFICIENT_STORAGE = TPM_ERROR (8),		/**< There is not enough storage space for the TPM. */
	TPM_INCOMPLETE_WRITE = TPM_ERROR (9),			/**< Write to storage only partially completed. */
};
