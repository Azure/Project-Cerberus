...
struct spdm_get_capabilities_flags_format {
	...
	uint8_t handshake_in_the_clear_cap:1;						/**< Supports communciation with messages exchanged during the Session Handshake Phase in the clear */
 ...
 */
#define	spdm_get_challenge_resp_measurement_summary_hash_max_len(max_response, cert_chain_hash_len, sig_len, opaque_data_len)	\
	(max_response - \
 ...
 * @param measurement_summary_hash Length of measurement summary hash
 */
#define	spdm_get_challenge_resp_opaque_len_ptr(resp, cert_chain_hash_len, measurement_summary_hash)	\
 ...
 * @param measurement_summary_hash Length of measurement summary hash
 */
#define	spdm_get_challenge_resp_signature_length(resp, cert_chain_hash_len, len, measurement_summary_hash) \
 ...
 */
#define	spdm_get_measurements_rq_length(rq)						\
	(sizeof (struct spdm_get_measurements_request) + sizeof (uint8_t) + \
		((rq)->sig_required * SPDM_NONCE_LEN))

/**
 * Get the buffer containing the nonce in SPDM get measurements request
 *
 * @param rq Buffer containing struct spdm_get_measurements_request
 */
...
/**
 * Get pointer containing slot ID buffer in SPDM get measurements request
 *
...
enum {
	...
	SPDM_ERROR_UNSPECIFIED = 0x05,								/**< Unspecified error occured */
