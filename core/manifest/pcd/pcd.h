...
struct pcd {
	 ...
	 */
	int (*get_platform_id) (struct pcd *pcd, char **id);
 ...
 */
enum {
	PCD_INVALID_ARGUMENT = PCD_ERROR (0),			/**< Input parameter is null or not valid. */
	PCD_NO_MEMORY = PCD_ERROR (1),					/**< Memory allocation failed. */
	PCD_BUF_TOO_SMALL = PCD_ERROR (2),				/**< The buffer provided is too small for requested contents. */
	PCD_UNKNOWN_COMPONENT = PCD_ERROR (3),			/**< The component identifier is not present in the PCD. */
	PCD_INVALID_SEG_LEN = PCD_ERROR (4),			/**< Segment in PCD has invalid length. */
	PCD_INVALID_SEG_HDR_LEN = PCD_ERROR (5),		/**< Segment header in PCD has invalid length. */
	PCD_INVALID_PORT = PCD_ERROR (6),				/**< Port not found in PCD. */
};
