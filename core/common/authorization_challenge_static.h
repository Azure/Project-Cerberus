 ...
 */
#define	authorization_challenge_static_init(state_ptr, token_ptr, auth_hash_arg) { \
 ...
 * use a nonce of AUTHORIZATION_CHALLENGE_NONCE_LENGTH bytes and an additional 4 bytes of data for
 * the uint32_t tag.
 * @param auth_hash_arg Hash algorithm to use for signature verification of authorized tokens.
 * @param tag The identifying tag value to include as part of the authorization token.
 */
#define	authorization_challenge_static_init_with_tag(state_ptr, token_ptr, auth_hash_arg, tag_arg) \
	{ \
		.base = AUTHORIZATION_CHALLENGE_API_INIT, \
		.state = state_ptr, \
		.token = token_ptr, \
