 ...
 *
 * @param interface_mctp The MCTP interface to initialize.
...
{
	if ((interface_mctp == NULL) || (cmd_interface == NULL) || (device_mgr == NULL)) {
		return MCTP_PROTOCOL_INVALID_ARGUMENT;
	}

	memset (interface_mctp, 0, sizeof (struct mctp_interface));

	interface_mctp->device_manager = device_mgr;
	interface_mctp->cmd_interface = cmd_interface;
	interface_mctp->eid = eid;
	interface_mctp->pci_vendor_id = pci_vid;
	interface_mctp->protocol_version = protocol_version;

	interface_mctp->req_buffer.data =
		&interface_mctp->msg_buffer[sizeof (interface_mctp->msg_buffer) - MCTP_PROTOCOL_MAX_MESSAGE_BODY];
	interface_mctp->resp_buffer.data = interface_mctp->msg_buffer;
 ...
 *
 * @param interface_mctp The MCTP interface to deinitialize
 */
void mctp_interface_deinit (struct mctp_interface *interface_mctp)
{
	if (interface_mctp != NULL) {
		memset (interface_mctp, 0, sizeof (struct mctp_interface));
	}
}
 ...
 *
 * @param interface_mctp The MCTP interface to assign the channel id.
...
int mctp_interface_set_channel_id (struct mctp_interface *interface_mctp, int channel_id)
{
	...
	interface_mctp->channel_id = channel_id;
 ...
 *
 * @param interface_mctp MCTP interface instance.
 ...
 */
static int mctp_interface_generate_error_packet (struct mctp_interface *interface_mctp,
	...
	uint8_t dest_eid, uint8_t msg_tag, uint8_t response_addr, uint8_t source_addr, uint8_t cmd_set)
{
	...
	if (error_code != CERBERUS_PROTOCOL_NO_ERROR) {
		debug_log_create_entry (DEBUG_LOG_SEVERITY_INFO, DEBUG_LOG_COMPONENT_MCTP,
			MCTP_LOGGING_CHANNEL, interface_mctp->channel_id, 0);
	...
	if (dest_eid != interface_mctp->eid) {
		return 0;
	}

	mctp_interface_reset_message_processing (interface_mctp);

	interface_mctp->req_buffer.max_response = MCTP_PROTOCOL_MIN_TRANSMISSION_UNIT;
	status = interface_mctp->cmd_interface->generate_error_packet (interface_mctp->cmd_interface,
		&interface_mctp->req_buffer, error_code, error_data, cmd_set);
	if (ROT_IS_ERROR (status)) {
		return status;
	}

	if (interface_mctp->req_buffer.length > MCTP_PROTOCOL_MIN_TRANSMISSION_UNIT) {
		return MCTP_PROTOCOL_MSG_TOO_LARGE;
	}

	status = mctp_protocol_construct (interface_mctp->req_buffer.data, interface_mctp->req_buffer.length,
		interface_mctp->resp_buffer.data, sizeof (interface_mctp->msg_buffer), source_addr, src_eid,
		dest_eid, true, true, 0, msg_tag, MCTP_PROTOCOL_TO_RESPONSE, response_addr,
		&interface_mctp->msg_type);
	if (ROT_IS_ERROR (status)) {
		return status;
	}

	interface_mctp->resp_buffer.msg_size = status;
	interface_mctp->resp_buffer.pkt_size = status;
	interface_mctp->resp_buffer.dest_addr = response_addr;

	*message = &interface_mctp->resp_buffer;
	return 0;
}
	...
	struct cmd_message **tx_message)
{
	...
	if ((interface_mctp == NULL) || (rx_packet == NULL) || (tx_message == NULL)) {
		return MCTP_PROTOCOL_INVALID_ARGUMENT;
	}
	...
	*tx_message = NULL;
	...
	status = mctp_protocol_interpret (rx_packet->data, rx_packet->pkt_size, rx_packet->dest_addr,
		...
		&packet_seq, &crc, &interface_mctp->msg_type);
	...
	if (status != 0) {
		...
		if ((status == MCTP_PROTOCOL_INVALID_MSG) || (status == MCTP_PROTOCOL_UNSUPPORTED_MSG)) {
			return mctp_interface_generate_error_packet (interface_mctp, tx_message,
	...
	if (som) {
		interface_mctp->req_buffer.length = 0;
		interface_mctp->req_buffer.source_eid = src_eid;
		interface_mctp->req_buffer.target_eid = dest_eid;
		interface_mctp->start_packet_len = payload_len;
		interface_mctp->req_buffer.channel_id = interface_mctp->channel_id;
		interface_mctp->packet_seq = 0;
		interface_mctp->msg_tag = msg_tag;
	}
	else if (interface_mctp->start_packet_len == 0) {
	...
	else if (packet_seq != interface_mctp->packet_seq) {
		return mctp_interface_generate_error_packet (interface_mctp, tx_message,
			CERBERUS_PROTOCOL_ERROR_OUT_OF_SEQ_WINDOW, 0, src_eid, dest_eid, msg_tag, response_addr,
			rx_packet->dest_addr, cmd_set);
	}
	else if (msg_tag != interface_mctp->msg_tag) {
		return mctp_interface_generate_error_packet (interface_mctp, tx_message,
			CERBERUS_PROTOCOL_ERROR_INVALID_REQ, 0, src_eid, dest_eid, msg_tag, response_addr,
			rx_packet->dest_addr, cmd_set);
	}
	else if (src_eid != interface_mctp->req_buffer.source_eid) {
		return 0;
	}
	else {
		if (((int) payload_len != interface_mctp->start_packet_len) &&
		   !(eom && ((int) payload_len < interface_mctp->start_packet_len))) {
	...
	if ((payload_len + interface_mctp->req_buffer.length) > MCTP_PROTOCOL_MAX_MESSAGE_BODY) {
		return mctp_interface_generate_error_packet (interface_mctp, tx_message,
			CERBERUS_PROTOCOL_ERROR_MSG_OVERFLOW, payload_len + interface_mctp->req_buffer.length,
	...
	memcpy (&interface_mctp->req_buffer.data[interface_mctp->req_buffer.length], payload, payload_len);
	interface_mctp->req_buffer.length += payload_len;
	interface_mctp->packet_seq = (interface_mctp->packet_seq + 1) % 4;
	...
	if (eom) {
		...
		if (MCTP_PROTOCOL_IS_CONTROL_MSG (interface_mctp->msg_type)) {
			interface_mctp->req_buffer.max_response = MCTP_PROTOCOL_MIN_TRANSMISSION_UNIT;
			status = mctp_interface_control_process_request (interface_mctp,	&interface_mctp->req_buffer,
				source_addr);
			if (status != 0) {
				debug_log_create_entry (DEBUG_LOG_SEVERITY_ERROR, DEBUG_LOG_COMPONENT_MCTP,
					MCTP_LOGGING_CONTROL_FAIL, status, interface_mctp->channel_id);
				return status;
			}
		}
		else if (MCTP_PROTOCOL_IS_VENDOR_MSG (interface_mctp->msg_type)) {
			header = (struct cerberus_protocol_header*) interface_mctp->req_buffer.data;
			cmd_set = header->rq;

			interface_mctp->req_buffer.max_response = device_manager_get_max_message_len_by_eid (
				interface_mctp->device_manager, src_eid);
			status = interface_mctp->cmd_interface->process_request (interface_mctp->cmd_interface,
				&interface_mctp->req_buffer);

			/* Regardless of the processing status, check to see if the timeout needs adjusting. */
			if (rx_packet->timeout_valid && interface_mctp->req_buffer.crypto_timeout) {
			...
			if (status == CMD_HANDLER_ERROR_MESSAGE) {
				if (interface_mctp->req_buffer.length == sizeof (struct cerberus_protocol_error)) {
					struct cerberus_protocol_error *error_msg =
						(struct cerberus_protocol_error*) interface_mctp->req_buffer.data;
			...
			if (status == ATTESTATION_START_TEST_ESCAPE_SEQ) {
				...
				status = device_manager_get_device_addr (interface_mctp->device_manager, device_num);
				if (!ROT_IS_ERROR (status)) {
					response_addr = status;
					status = interface_mctp->cmd_interface->issue_request (interface_mctp->cmd_interface,
						CERBERUS_PROTOCOL_GET_DIGEST, NULL, interface_mctp->req_buffer.data,
						MCTP_PROTOCOL_MAX_MESSAGE_BODY);
					if (!ROT_IS_ERROR (status)) {
						interface_mctp->req_buffer.source_eid =
							device_manager_get_device_eid (interface_mctp->device_manager, device_num);
						interface_mctp->req_buffer.length = status;
						tag_owner = MCTP_PROTOCOL_TO_REQUEST;
						interface_mctp->req_buffer.new_request = true;
						status = 0;
					}
				}
			...
			else if (interface_mctp->req_buffer.length == 0) {
				return mctp_interface_generate_error_packet (interface_mctp, tx_message,
					CERBERUS_PROTOCOL_NO_ERROR, status, src_eid, dest_eid, msg_tag, response_addr,
					rx_packet->dest_addr, cmd_set);
			}

			if (interface_mctp->req_buffer.length >
				device_manager_get_max_message_len_by_eid (interface_mctp->device_manager, src_eid)) {
				return mctp_interface_generate_error_packet (interface_mctp, tx_message,
		...
		if (interface_mctp->req_buffer.new_request) {
			tag_owner = MCTP_PROTOCOL_TO_REQUEST;
		}
		else {
		...
		if (interface_mctp->req_buffer.length > 0) {
			interface_mctp->packet_seq = 0;
			i_buf = 0;
			som = true;

			max_packet = device_manager_get_max_transmission_unit_by_eid (interface_mctp->device_manager,
				src_eid);
			n_packets = MCTP_PROTOCOL_PACKETS_IN_MESSAGE (interface_mctp->req_buffer.length, max_packet);

			interface_mctp->resp_buffer.msg_size = 0;
			for (i_packet = 0; i_packet < n_packets; ++i_packet) {
				eom = (i_packet == (n_packets - 1));
				payload_len = (interface_mctp->req_buffer.length > max_packet) ?
					max_packet : interface_mctp->req_buffer.length;

				status = mctp_protocol_construct (&interface_mctp->req_buffer.data[i_buf], payload_len,
					&interface_mctp->resp_buffer.data[interface_mctp->resp_buffer.msg_size],
					sizeof (interface_mctp->msg_buffer) - interface_mctp->resp_buffer.msg_size,
					rx_packet->dest_addr, interface_mctp->req_buffer.source_eid,
					interface_mctp->req_buffer.target_eid, som, eom, interface_mctp->packet_seq,
					interface_mctp->msg_tag, tag_owner, response_addr, &interface_mctp->msg_type);
				if (ROT_IS_ERROR (status)) {
					if (MCTP_PROTOCOL_IS_VENDOR_MSG (interface_mctp->msg_type)) {
						return mctp_interface_generate_error_packet (interface_mctp, tx_message,
							CERBERUS_PROTOCOL_ERROR_UNSPECIFIED, status, src_eid, dest_eid, msg_tag,
							response_addr, rx_packet->dest_addr, cmd_set);
					}
			...
			interface_mctp->msg_tag = (interface_mctp->msg_tag + 1) % 8;

			*tx_message = &interface_mctp->resp_buffer;
		}
		else {
			*tx_message = NULL;
		}
	...
	uint8_t *buf, size_t buf_len, uint8_t msg_type)
{
	...
	if ((interface_mctp == NULL) || (buf == NULL)) {
		return MCTP_PROTOCOL_INVALID_ARGUMENT;
	}

	if (msg_type == MCTP_PROTOCOL_MSG_TYPE_VENDOR_DEF) {
		status = interface_mctp->cmd_interface->issue_request (interface_mctp->cmd_interface, command_id,
			request_params, msg_buffer,
			device_manager_get_max_message_len_by_eid (interface_mctp->device_manager, dest_eid));
		if (ROT_IS_ERROR (status)) {
			return status;
		}
	}
		...
		status = mctp_interface_control_issue_request (interface_mctp, command_id, request_params,
	...
	status = mctp_protocol_construct (msg_buffer, status, buf, buf_len, src_addr, dest_eid, src_eid,
		true, true, 0, interface_mctp->msg_tag, MCTP_PROTOCOL_TO_REQUEST, dest_addr, &msg_type);
	if (ROT_IS_ERROR (status)) {
		return status;
	}
