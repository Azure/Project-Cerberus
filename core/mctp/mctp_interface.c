...
static int mctp_interface_generate_error_packet (struct mctp_interface *interface,
	...
	uint8_t cmd_set)
{
	uint8_t error_buf[CERBERUS_PROTOCOL_MIN_MSG_LEN + 5];
	struct cerberus_protocol_header *header = (struct cerberus_protocol_header*) error_buf;
	int status;
	...
	*packets = platform_calloc (1, sizeof (struct cmd_packet));
	...
	error_buf[CERBERUS_PROTOCOL_MIN_MSG_LEN] = error_code;

	memcpy (&error_buf[CERBERUS_PROTOCOL_MIN_MSG_LEN + 1], &error_data, sizeof (error_data));

	status = mctp_protocol_construct (error_buf, sizeof (error_buf), &(*packets)[0].data[0],
	...
	struct cmd_packet **tx_packets, size_t *num_packets)
{
	...
	*tx_packets = NULL;
	...
	status = mctp_protocol_interpret (rx_packet->data, rx_packet->pkt_size, &source_addr, &som,
		&eom, &src_eid, &dest_eid, &payload, &payload_len, &msg_tag, &packet_seq, &crc,
		rx_packet->dest_addr, &interface->msg_type);
	...
	if (eom) {
		if ((interface->msg_type & MCTP_PROTOCOL_MSG_TYPE_SET_MASK) ==
		...
		else if ((interface->msg_type & MCTP_PROTOCOL_MSG_TYPE_SET_MASK) ==
			...
			if (((uint16_t) status) == CMD_ERROR_MESSAGE_ESCAPE_SEQ) {
				if (interface->msg_buffer.length == (CERBERUS_PROTOCOL_MIN_MSG_LEN + 5)) {
					error_code = interface->msg_buffer.data[CERBERUS_PROTOCOL_MIN_MSG_LEN];
					memcpy (&error_data,
						&interface->msg_buffer.data[CERBERUS_PROTOCOL_MIN_MSG_LEN + 1],
						sizeof (error_data));
			...
			if (((uint16_t) status) == ATTESTATION_START_TEST_ESCAPE_SEQ) {
			...
			if (interface->msg_buffer.length > MCTP_PROTOCOL_MAX_PAYLOAD_PER_MSG) {
				return mctp_interface_generate_error_packet (interface, tx_packets, num_packets,
		...
		if (interface->msg_buffer.length > 0) {
			...
			n_packets = ceil (interface->msg_buffer.length /
				(1.0 * MCTP_PROTOCOL_MAX_PAYLOAD_PER_PKT));
			*tx_packets = platform_calloc (n_packets, sizeof (struct cmd_packet));

			if ((*tx_packets == NULL) && ((interface->msg_type & MCTP_PROTOCOL_MSG_TYPE_SET_MASK) ==
				MCTP_PROTOCOL_MSG_TYPE_VENDOR_DEF)) {
				return mctp_interface_generate_error_packet (interface, tx_packets, num_packets,
			...
			for (i_packet = 0; i_packet < n_packets; ++i_packet) {
				...
				payload_len = (interface->msg_buffer.length > MCTP_PROTOCOL_MAX_PAYLOAD_PER_PKT) ?
					MCTP_PROTOCOL_MAX_PAYLOAD_PER_PKT : interface->msg_buffer.length;

				status = mctp_protocol_construct (&interface->msg_buffer.data[i_buf], payload_len,
					&(*tx_packets)[i_packet].data[0], sizeof ((*tx_packets)[i_packet].data),
					rx_packet->dest_addr, interface->msg_buffer.source_eid,
					interface->msg_buffer.target_eid, som, eom, interface->packet_seq,
					interface->msg_tag, tag_owner, response_addr, &interface->msg_type);

				if ((ROT_IS_ERROR (status)) &&
					((interface->msg_type & MCTP_PROTOCOL_MSG_TYPE_SET_MASK) ==
						MCTP_PROTOCOL_MSG_TYPE_VENDOR_DEF)) {
...
{
	uint8_t msg_buffer[MCTP_PROTOCOL_MAX_PAYLOAD_PER_PKT] = {0};
	int status;
	...
	if (msg_type == MCTP_PROTOCOL_MSG_TYPE_VENDOR_DEF) {
		status = interface->cmd_interface->issue_request (interface->cmd_interface, command_id,
			request_params, msg_buffer, sizeof (msg_buffer));
		if (ROT_IS_ERROR (status)) {
			return status;
		}
	}
