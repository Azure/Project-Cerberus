...
#include <stdint.h>
#include <string.h>
#include "attestation_responder_mock.h"


static int attestation_responder_mock_get_digests (struct attestation_responder *attestation,
	uint8_t slot_num, uint8_t *buf, size_t buf_len, uint8_t *num_cert)
{
	struct attestation_responder_mock *mock = (struct attestation_responder_mock*) attestation;
...
static int attestation_responder_mock_generate_ecdh_seed (struct attestation_responder *attestation,
...
{
	if (func == attestation_responder_mock_aux_attestation_unseal) {
		return 14;
	}
	else if (func == attestation_responder_mock_aux_decrypt) {
		return 7;
	}
	else if (func == attestation_responder_mock_generate_ecdh_seed) {
		return 5;
	}
	else if (func == attestation_responder_mock_get_digests) {
		return 4;
	}
	else if (func == attestation_responder_mock_get_certificate) {
		return 3;
	}
	else if (func == attestation_responder_mock_challenge_response) {
		return 2;
	}
	else {
		return 0;
...
static const char* attestation_responder_mock_func_name_map (void *func)
{
	if (func == attestation_responder_mock_get_digests) {
		return "get_digests";
	}
	else if (func == attestation_responder_mock_get_certificate) {
	...
	else if (func == attestation_responder_mock_aux_attestation_unseal) {
...
static const char* attestation_responder_mock_arg_name_map (void *func, int arg)
{
	if (func == attestation_responder_mock_get_digests) {
		switch (arg) {
			case 0:
...
int attestation_responder_mock_init (struct attestation_responder_mock *mock)
{
	...
	memset (mock, 0, sizeof (struct attestation_responder_mock));
	...
	mock->base.get_digests = attestation_responder_mock_get_digests;
	mock->base.get_certificate = attestation_responder_mock_get_certificate;
	mock->base.challenge_response = attestation_responder_mock_challenge_response;
	mock->base.aux_attestation_unseal = attestation_responder_mock_aux_attestation_unseal;
	mock->base.aux_decrypt = attestation_responder_mock_aux_decrypt;
	mock->base.generate_ecdh_seed = attestation_responder_mock_generate_ecdh_seed;

	mock->mock.func_arg_count = attestation_responder_mock_func_arg_count;
	mock->mock.func_name_map = attestation_responder_mock_func_name_map;
	mock->mock.arg_name_map = attestation_responder_mock_arg_name_map;
...
int attestation_responder_mock_validate_and_release (struct attestation_responder_mock *mock)
{
	...
	if (mock != NULL) {
		...
		attestation_responder_mock_release (mock);
	}
