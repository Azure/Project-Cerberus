	...
	CuTest *test)
{
	...
	rx.data[15] = 0x05;
	rx.data[16] = 0x06;
	rx.data[17] = checksum_crc8 (0xC4, rx.data, 17);
	rx.pkt_size = 18;
	rx.dest_addr = 0x62;
	...
	CuAssertIntEquals (test, 10, tx->msg_size);
	CuAssertIntEquals (test, tx->msg_size, tx->pkt_size);
	CuAssertIntEquals (test, 0x39, tx->dest_addr);

	header = (struct mctp_base_protocol_transport_header*) tx->data;

	CuAssertIntEquals (test, 0x0F, header->cmd_code);
	CuAssertIntEquals (test, 7, header->byte_count);
	CuAssertIntEquals (test, 0xC5, header->source_addr);
	CuAssertIntEquals (test, 0x0A, header->destination_eid);
	CuAssertIntEquals (test, 0x0B, header->source_eid);
	CuAssertIntEquals (test, 1, header->som);
	CuAssertIntEquals (test, 1, header->eom);
	...
	CuAssertIntEquals (test, 0, header->packet_seq);
	CuAssertIntEquals (test, checksum_crc8 (0x72, tx->data, tx->pkt_size - 1),
		tx->data[tx->pkt_size - 1]);
...
static void mctp_interface_test_process_packet_max_response (CuTest *test)
{
	...
	for (i = 0; i < max_packets - 1; i++) {
		header = (struct mctp_base_protocol_transport_header*) &tx->data[
			i * MCTP_BASE_PROTOCOL_MAX_PACKET_LEN];
		...
		status = testing_validate_array (&response.data[i *
			MCTP_BASE_PROTOCOL_MAX_TRANSMISSION_UNIT],
...
static void mctp_interface_test_send_request_message_max_size (CuTest *test)
{
	...
	memcpy (&tx_packet[i].data[7], &request.payload[MCTP_BASE_PROTOCOL_MAX_TRANSMISSION_UNIT * i],
		remain);
...
static void mctp_interface_test_send_request_message_max_size_min_packets (CuTest *test)
{
	...
	memcpy (&tx_packet[i].data[7], &request.payload[MCTP_BASE_PROTOCOL_MIN_TRANSMISSION_UNIT * i],
		remain);
...
static void mctp_interface_test_send_request_message_max_response (CuTest *test)
{
	...
	for (i = 0; i < ARRAY_SIZE (rx_packet) - 1; i++) {
		...
		memcpy (&rx_packet[i].data[7], &rx_expected[MCTP_BASE_PROTOCOL_MAX_TRANSMISSION_UNIT * i],
			MCTP_BASE_PROTOCOL_MAX_TRANSMISSION_UNIT);
...
static void
mctp_interface_test_send_request_message_no_response_wait_receive_response_descriptor_not_null (
	CuTest *test)
{
	struct mctp_interface_testing mctp;
	struct mctp_base_protocol_transport_header *header;
...
static void
mctp_interface_test_send_request_message_no_response_wait_receive_response_drop_unexpected_response_msg_tags
(
	CuTest *test)
{
	struct mctp_interface_testing mctp;
...
static void
mctp_interface_test_send_request_message_no_response_wait_receive_response_drop_unexpected_response_msg_type
(
	CuTest *test)
{
	struct mctp_interface_testing mctp;
...
static void
mctp_interface_test_send_request_message_no_response_wait_receive_response_drop_request_packet_no_som
(
	CuTest *test)
{
	struct mctp_interface_testing mctp;
...
static void mctp_interface_test_issue_request_then_process_response_fail (CuTest *test)
{
	...
	status = mock_expect (&mctp.cmd_cerberus.mock, mctp.cmd_cerberus.base.process_response,
		&mctp.cmd_cerberus, CMD_HANDLER_NO_MEMORY,
		MOCK_ARG_VALIDATOR_DEEP_COPY (cmd_interface_mock_validate_request, &response,
		sizeof (response), cmd_interface_mock_save_request, cmd_interface_mock_free_request));
	CuAssertIntEquals (test, 0, status);
	...
	CuTest *test)
{
	...
	status = mock_expect (&mctp.cmd_mctp.mock, mctp.cmd_mctp.base.process_response,	&mctp.cmd_mctp,
		0,
		MOCK_ARG_VALIDATOR_DEEP_COPY (cmd_interface_mock_validate_request, &response,
		sizeof (response), cmd_interface_mock_save_request, cmd_interface_mock_free_request));
	CuAssertIntEquals (test, 0, status);
...
static void
mctp_interface_test_issue_request_mctp_control_then_process_packet_response_mctp_control_unsupported
(
	CuTest *test)
{
	struct mctp_interface_testing mctp;
