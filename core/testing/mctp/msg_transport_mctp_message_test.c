...
struct msg_transport_mctp_message_testing {
	...
	struct msg_transport_mctp_message test;			/**< Protocol handler being tested. */
};
...
static void msg_transport_mctp_message_test_get_max_message_overhead (CuTest *test)
{
	...
	status = mctp.test.base.get_max_message_overhead (&mctp.test.base, dest_id);
...
static void msg_transport_mctp_message_test_get_max_message_overhead_null (CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 8;
	int status;
	...
	status = mctp.test.base.get_max_message_overhead (NULL, dest_id);
	CuAssertIntEquals (test, MSG_TRANSPORT_INVALID_ARGUMENT, status);

	msg_transport_mctp_message_testing_release (test, &mctp);
}

static void msg_transport_mctp_message_test_get_max_message_overhead_transport_error (CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 51;
	int status;
	...
	status = mock_expect (&mctp.mctp_transport.mock,
		mctp.mctp_transport.base.get_max_message_overhead, &mctp.mctp_transport,
...
static void msg_transport_mctp_message_test_get_max_message_payload_length (CuTest *test)
{
	...
	status = mctp.test.base.get_max_message_payload_length (&mctp.test.base, dest_id);
	CuAssertIntEquals (test, payload - 1, status);

	msg_transport_mctp_message_testing_release (test, &mctp);
}

static void msg_transport_mctp_message_test_get_max_message_payload_length_less_than_overhead (
	CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 31;
...
static void msg_transport_mctp_message_test_get_max_message_payload_length_null (CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 3;
	int status;
	...
	status = mctp.test.base.get_max_message_payload_length (NULL, dest_id);
	CuAssertIntEquals (test, MSG_TRANSPORT_INVALID_ARGUMENT, status);

	msg_transport_mctp_message_testing_release (test, &mctp);
}

static void msg_transport_mctp_message_test_get_max_message_payload_length_transport_error (
	CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 3;
	...
	status = mock_expect (&mctp.mctp_transport.mock,
		mctp.mctp_transport.base.get_max_message_payload_length, &mctp.mctp_transport,
...
static void msg_transport_mctp_message_test_get_max_encapsulated_message_length (CuTest *test)
{
	...
	status = mctp.test.base.get_max_encapsulated_message_length (&mctp.test.base, dest_id);
...
static void msg_transport_mctp_message_test_get_max_encapsulated_message_length_null (CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 12;
	int status;
	...
	status = mctp.test.base.get_max_encapsulated_message_length (NULL, dest_id);
	CuAssertIntEquals (test, MSG_TRANSPORT_INVALID_ARGUMENT, status);

	msg_transport_mctp_message_testing_release (test, &mctp);
}

static void msg_transport_mctp_message_test_get_max_encapsulated_message_length_transport_error (
	CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 12;
	...
	status = mock_expect (&mctp.mctp_transport.mock,
		mctp.mctp_transport.base.get_max_encapsulated_message_length, &mctp.mctp_transport,
...
static void msg_transport_mctp_message_test_get_buffer_overhead (CuTest *test)
{
	...
	status = mctp.test.base.get_buffer_overhead (&mctp.test.base, dest_id, length);
	CuAssertIntEquals (test, overhead + 1, status);
...
static void msg_transport_mctp_message_test_get_buffer_overhead_null (CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 8;
	size_t length = 128;
	...
	status = mctp.test.base.get_buffer_overhead (NULL, dest_id, length);
	CuAssertIntEquals (test, MSG_TRANSPORT_INVALID_ARGUMENT, status);

	msg_transport_mctp_message_testing_release (test, &mctp);
}

static void msg_transport_mctp_message_test_get_buffer_overhead_transport_error (CuTest *test)
{
	struct msg_transport_mctp_message_testing mctp;
	uint8_t dest_id = 8;
	size_t length = 128;
	...
	status = mock_expect (&mctp.mctp_transport.mock,
		mctp.mctp_transport.base.get_buffer_overhead, &mctp.mctp_transport,
...
static void msg_transport_mctp_message_test_send_request_message (CuTest *test)
{
	...
	status = mctp.test.base.send_request_message (&mctp.test.base, &request, timeout, &response);
	CuAssertIntEquals (test, 0, status);
...
static void msg_transport_mctp_message_test_send_request_message_null (CuTest *test)
{
	...
	status = mctp.test.base.send_request_message (NULL, &request, timeout, &response);
	CuAssertIntEquals (test, MSG_TRANSPORT_INVALID_ARGUMENT, status);

	status = mctp.test.base.send_request_message (&mctp.test.base, NULL, timeout, &response);
	CuAssertIntEquals (test, MSG_TRANSPORT_INVALID_ARGUMENT, status);

	status = mctp.test.base.send_request_message (&mctp.test.base, &request, timeout, NULL);
	CuAssertIntEquals (test, MSG_TRANSPORT_INVALID_ARGUMENT, status);
...
TEST (msg_transport_mctp_message_test_get_max_message_overhead_null);
TEST (msg_transport_mctp_message_test_get_max_message_overhead_transport_error);
TEST (msg_transport_mctp_message_test_get_max_message_payload_length);
TEST (msg_transport_mctp_message_test_get_max_message_payload_length_less_than_overhead);
TEST (msg_transport_mctp_message_test_get_max_message_payload_length_static_init);
TEST (msg_transport_mctp_message_test_get_max_message_payload_length_null);
TEST (msg_transport_mctp_message_test_get_max_message_payload_length_transport_error);
...
TEST (msg_transport_mctp_message_test_get_max_encapsulated_message_length_null);
TEST (msg_transport_mctp_message_test_get_max_encapsulated_message_length_transport_error);
...
TEST (msg_transport_mctp_message_test_get_buffer_overhead_null);
TEST (msg_transport_mctp_message_test_get_buffer_overhead_transport_error);
