...
static void aes_mbedtls_test_encrypt_with_add_data (CuTest *test)
{
	struct aes_engine_mbedtls_testing engine;
	int status;
	uint8_t ciphertext[AES_CIPHERTEXT_LEN * 2];
...
/* TODO: mbedTLS doesn't support GCM operations without a tag. */
#if 0
static void aes_mbedtls_test_encrypt_with_add_data_no_tag (CuTest *test)
{
	struct aes_engine_mbedtls_testing engine;
	int status;
	uint8_t ciphertext[AES_CIPHERTEXT_LEN * 2];
...
static void aes_mbedtls_test_encrypt_with_add_data_additional_data_static_init (CuTest *test)
{
	struct aes_engine_mbedtls_testing engine = {
		.test = aes_mbedtls_static_init (&engine.state)
	};
...
static void aes_mbedtls_test_decrypt_data_static_init (CuTest *test)
{
	struct aes_engine_mbedtls_testing engine = {
		.test = aes_mbedtls_static_init (&engine.state)
	};
...
static void aes_mbedtls_test_decrypt_with_add_data_no_additional_data (CuTest *test)
{
	struct aes_engine_mbedtls_testing engine;
	int status;
...
TEST (aes_mbedtls_test_encrypt_with_add_data);
...
TEST (aes_mbedtls_test_encrypt_with_add_data_additional_data_static_init);
...
TEST (aes_mbedtls_test_decrypt_data_static_init);
...
TEST (aes_mbedtls_test_decrypt_with_add_data_no_additional_data);
