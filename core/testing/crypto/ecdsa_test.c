...
const uint8_t ECDSA_TESTING_DETERMINISTIC_K_DRBG_ECC521_K_OUT_0[] = {
	0x01, 0xbb, 0x97, 0x6d, 0x90, 0x15, 0x5c, 0xc3, 0x96, 0x7f, 0x6d, 0xf9, 0x79, 0x35, 0x78, 0x42,
...
const uint8_t ECDSA_TESTING_DETERMINISTIC_K_DRBG_ECC521_K_OUT_1[] = {
	0x00, 0xa3, 0xfc, 0x33, 0x9b, 0xf6, 0x78, 0x14, 0x77, 0x3b, 0xe1, 0x40, 0x09, 0xd4, 0xda, 0xe1,
...
/**
 * ECC P-521 k value from the RFC 6979 test vectors, left shifted by 7 bits.
 */
const uint8_t ECDSA_TESTING_DETERMINISTIC_K_RFC6979_ECC521_K_OUT_SHIFTED[] = {
	0x01, 0x71, 0x75, 0x03, 0x8f, 0xc0, 0x88, 0x6e, 0x13, 0x44, 0x16, 0xa6, 0xaf, 0x57, 0x03, 0x10,
	0xd1, 0x92, 0xb7, 0xe4, 0x42, 0x3f, 0xdc, 0x81, 0x17, 0x15, 0xbe, 0x94, 0x73, 0x78, 0x80, 0xcc,
	0x58, 0xab, 0xa7, 0xee, 0x81, 0xd4, 0x82, 0x9e, 0x04, 0x50, 0xc2, 0xa5, 0x0b, 0x45, 0x52, 0xd2,
	0xba, 0x38, 0x76, 0x4b, 0xee, 0xae, 0x70, 0x48, 0x09, 0x27, 0x7a, 0x95, 0x17, 0xbf, 0x65, 0xff,
	0xe9, 0xcb
};
...
static void ecdsa_test_deterministic_k_drbg_instantiate_ecc256_sha256 (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA256, SHA256_TEST_HASH,
		SHA256_HASH_LENGTH, ECC_PRIVKEY, ECC_KEY_LENGTH_256, &drbg);
	CuAssertIntEquals (test, 0, status);
...
static void ecdsa_test_deterministic_k_drbg_instantiate_ecc384_sha384 (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA384, SHA384_TEST_HASH,
		SHA384_HASH_LENGTH, ECC384_PRIVKEY, ECC_KEY_LENGTH_384, &drbg);
...
static void ecdsa_test_deterministic_k_drbg_instantiate_ecc521_sha512 (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA512, SHA512_TEST_HASH,
		SHA512_HASH_LENGTH, ECC521_PRIVKEY, ECC_KEY_LENGTH_521, &drbg);
...
static void ecdsa_test_deterministic_k_drbg_instantiate_null (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_instantiate (NULL, HMAC_SHA256, SHA256_TEST_HASH,
		SHA256_HASH_LENGTH, ECC_PRIVKEY, ECC_KEY_LENGTH_256, &drbg);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);

	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA256, NULL,
		SHA256_HASH_LENGTH, ECC_PRIVKEY, ECC_KEY_LENGTH_256, &drbg);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);

	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA256, SHA256_TEST_HASH,
		SHA256_HASH_LENGTH, NULL, ECC_KEY_LENGTH_256, &drbg);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);

	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA256, SHA256_TEST_HASH,
		SHA256_HASH_LENGTH, ECC_PRIVKEY, ECC_KEY_LENGTH_256, NULL);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
...
static void ecdsa_test_deterministic_k_drbg_instantiate_invalid_hmac_algorithm (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_INVALID, SHA256_TEST_HASH,
		SHA256_HASH_LENGTH, ECC_PRIVKEY, ECC_KEY_LENGTH_256, &drbg);
...
static void ecdsa_test_deterministic_k_drbg_generate_null (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_generate (NULL, &drbg, k, sizeof (k));
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);

	status = ecdsa_deterministic_k_drbg_generate (&hash.base, NULL, k, sizeof (k));
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);

	status = ecdsa_deterministic_k_drbg_generate (&hash.base, &drbg, NULL, sizeof (k));
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
...
static void ecdsa_test_deterministic_k_drbg_rfc6979_test_vector_ecc256 (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA256,
		ECDSA_TESTING_DETERMINISTIC_K_RFC6979_SHA256_DIGEST, SHA256_HASH_LENGTH,
		ECDSA_TESTING_DETERMINISTIC_K_RFC6979_ECC256_PRIV_KEY, ECC_KEY_LENGTH_256, &drbg);
	CuAssertIntEquals (test, 0, status);
...
static void ecdsa_test_deterministic_k_drbg_rfc6979_test_vector_ecc384 (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA384,
		ECDSA_TESTING_DETERMINISTIC_K_RFC6979_SHA384_DIGEST, SHA384_HASH_LENGTH,
		ECDSA_TESTING_DETERMINISTIC_K_RFC6979_ECC384_PRIV_KEY, ECC_KEY_LENGTH_384, &drbg);
	CuAssertIntEquals (test, 0, status);
...
static void ecdsa_test_deterministic_k_drbg_rfc6979_test_vector_ecc521 (CuTest *test)
{
	...
	status = ecdsa_deterministic_k_drbg_instantiate (&hash.base, HMAC_SHA512,
		ECDSA_TESTING_DETERMINISTIC_K_RFC6979_SHA512_DIGEST, ECC_KEY_LENGTH_521,
		ECDSA_TESTING_DETERMINISTIC_K_RFC6979_ECC521_PRIV_KEY, ECC_KEY_LENGTH_521, &drbg);
	CuAssertIntEquals (test, 0, status);

	status = ecdsa_deterministic_k_drbg_generate (&hash.base, &drbg, k, sizeof (k));
	CuAssertIntEquals (test, 0, status);

	/* NOTE: This implementation does not map directly to the RFC test vectors for P-521 keys. */
	status = testing_validate_array (ECDSA_TESTING_DETERMINISTIC_K_RFC6979_ECC521_K_OUT_SHIFTED, k,
		ECC_KEY_LENGTH_521);
	CuAssertIntEquals (test, 0, status);
...
static void ecdsa_test_ecc_hw_sign_message_null (CuTest *test)
{
	...
	status = ecdsa_ecc_hw_sign_message (NULL, &ecdsa.hash.base, HASH_TYPE_SHA256,
		&ecdsa.rng.base, ECC_PRIVKEY, ECC_PRIVKEY_LEN, HASH_TESTING_FULL_BLOCK_1024,
		HASH_TESTING_FULL_BLOCK_1024_LEN, &signature);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);

	status = ecdsa_ecc_hw_sign_message (&ecdsa.ecc_hw.base, NULL, HASH_TYPE_SHA256,
		&ecdsa.rng.base, ECC_PRIVKEY, ECC_PRIVKEY_LEN, HASH_TESTING_FULL_BLOCK_1024,
		HASH_TESTING_FULL_BLOCK_1024_LEN, &signature);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
...
static void ecdsa_test_ecc_hw_sign_message_hash_finish_error (CuTest *test)
{
	...
	status |= mock_expect (&ecdsa.hash_mock.mock, ecdsa.hash_mock.base.update, &ecdsa.hash_mock,
		0, MOCK_ARG_PTR_CONTAINS (HASH_TESTING_FULL_BLOCK_1024, HASH_TESTING_FULL_BLOCK_1024_LEN),
		MOCK_ARG (HASH_TESTING_FULL_BLOCK_1024_LEN));
...
static void ecdsa_test_ecc_hw_sign_hash_no_rng (CuTest *test)
{
	...
	status = ecdsa_ecc_hw_sign_hash (&ecdsa.ecc_hw.base, &ecdsa.hash.base, HASH_TYPE_SHA256,
		NULL, ECC_PRIVKEY, ECC_PRIVKEY_LEN, &signature);
	CuAssertIntEquals (test, 0, status);
	CuAssertIntEquals (test, ECC_KEY_LENGTH_256, signature.length);
...
static void ecdsa_test_ecc_hw_sign_hash_null (CuTest *test)
{
	...
	status = ecdsa_ecc_hw_sign_hash (NULL, &ecdsa.hash.base, HASH_TYPE_SHA256,
		&ecdsa.rng.base, ECC_PRIVKEY, ECC_PRIVKEY_LEN, &signature);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);

	status = ecdsa_ecc_hw_sign_hash (&ecdsa.ecc_hw.base, NULL, HASH_TYPE_SHA256,
		&ecdsa.rng.base, ECC_PRIVKEY, ECC_PRIVKEY_LEN, &signature);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
...
static void ecdsa_test_ecc_hw_sign_hash_and_finish_null (CuTest *test)
{
	...
	status = ecdsa_ecc_hw_sign_hash_and_finish (NULL, &ecdsa.hash_mock.base,
		HASH_TYPE_SHA256, &ecdsa.rng.base, ECC_PRIVKEY, ECC_PRIVKEY_LEN, &signature);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
...
static void ecdsa_test_ecc_hw_sign_hash_and_finish_null_hash (CuTest *test)
{
	...
	status = ecdsa_ecc_hw_sign_hash_and_finish (&ecdsa.ecc_hw.base, NULL,
		HASH_TYPE_SHA256, &ecdsa.rng.base, ECC_PRIVKEY, ECC_PRIVKEY_LEN, &signature);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
...
static void ecdsa_test_ecc_hw_verify_message_hash_finish_error (CuTest *test)
{
	...
	status |= mock_expect (&ecdsa.hash_mock.mock, ecdsa.hash_mock.base.update, &ecdsa.hash_mock,
		0, MOCK_ARG_PTR_CONTAINS (message, strlen (message)), MOCK_ARG (strlen (message)));
...
static void ecdsa_test_ecc_hw_verify_hash_null (CuTest *test)
{
	...
	status = ecdsa_ecc_hw_verify_hash (NULL, &ecdsa.hash.base, HASH_TYPE_SHA256,
		&ECC_PUBKEY_POINT, &ECC_SIGNATURE_TEST_STRUCT);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
	...
	status = ecdsa_ecc_hw_verify_hash (&ecdsa.ecc_hw.base, &ecdsa.hash.base, HASH_TYPE_SHA256,
		NULL, &ECC_SIGNATURE_TEST_STRUCT);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
...
static void ecdsa_test_ecc_hw_verify_hash_and_finish_null (CuTest *test)
{
	...
	status = ecdsa_ecc_hw_verify_hash_and_finish (NULL, &ecdsa.hash_mock.base,
		HASH_TYPE_SHA256, &ECC_PUBKEY_POINT, &ECC_SIGNATURE_TEST_STRUCT);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
...
static void ecdsa_test_ecc_hw_verify_hash_and_finish_null_hash (CuTest *test)
{
	...
	status = ecdsa_ecc_hw_verify_hash_and_finish (&ecdsa.ecc_hw.base, NULL,
		HASH_TYPE_SHA256, &ECC_PUBKEY_POINT, &ECC_SIGNATURE_TEST_STRUCT);
	CuAssertIntEquals (test, ECDSA_INVALID_ARGUMENT, status);
