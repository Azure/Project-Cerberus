...
static void spdm_transcript_manager_test_static_init (CuTest *test)
{
	...
	const struct spdm_transcript_manager transcript_manager = spdm_transcript_manager_static_init (
		&testing.state, testing.hash_engine, ARRAY_SIZE (testing.hash_engine));
...
static void spdm_transcript_manager_test_static_init_invalid_params (CuTest *test)
{
	...
	/* state_ptr = NULL */
	const struct spdm_transcript_manager transcript_manager = spdm_transcript_manager_static_init (
		NULL, testing.hash_engine, ARRAY_SIZE (testing.hash_engine));
	status = spdm_transcript_manager_init_state (&transcript_manager);
	CuAssertIntEquals (test, SPDM_TRANSCRIPT_MANAGER_INVALID_ARGUMENT, status);

	/* hash_engine array = NULL */
	const struct spdm_transcript_manager transcript_manager2 = spdm_transcript_manager_static_init (
		&testing.state, NULL, ARRAY_SIZE (testing.hash_engine));
	status = spdm_transcript_manager_init_state (&transcript_manager2);
	CuAssertIntEquals (test, SPDM_TRANSCRIPT_MANAGER_INVALID_ARGUMENT, status);

	/* hash_engine_count = 0 */
	const struct spdm_transcript_manager transcript_manager3 = spdm_transcript_manager_static_init (
		&testing.state, testing.hash_engine, 0);
	...
	const struct spdm_transcript_manager transcript_manager4 = spdm_transcript_manager_static_init (
		&testing.state, testing.hash_engine, ARRAY_SIZE (testing.hash_engine));
...
static void spdm_transcript_manager_test_update_invalid_params (CuTest *test)
{
	...
	/* message_size = 0 */
	status = transcript_manager->update (transcript_manager, TRANSCRIPT_CONTEXT_TYPE_L1L2,
		message, 0, use_session_context, session_idx);
	CuAssertIntEquals (test, SPDM_TRANSCRIPT_MANAGER_INVALID_ARGUMENT, status);
...
static void spdm_transcript_manager_test_update_m1m2_SHA256 (CuTest *test)
{
	...
	const uint8_t expected_hash[] = { 0xd9, 0xe0, 0xd4, 0xc3, 0x85, 0x0a, 0xa1, 0x30, 0xf9, 0x09,
...
static void spdm_transcript_manager_test_update_m1m2_SHA384 (CuTest *test)
{
	...
	const uint8_t expected_hash[] = { 0xe1, 0x28, 0x84, 0x30, 0xfb, 0xb2, 0xd4, 0x63, 0xa1, 0xbd,
...
static void spdm_transcript_manager_test_update_m1m2_SHA512 (CuTest *test)
{
	...
	const uint8_t expected_hash[] = { 0x0a, 0x12, 0x15, 0xf3, 0x1a, 0xcd, 0x40, 0xae, 0x63, 0x89,
