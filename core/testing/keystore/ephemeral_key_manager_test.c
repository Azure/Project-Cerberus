...
static void ephemeral_key_manager_test_get_key_null (CuTest *test)
{
...
static void ephemeral_key_manager_test_get_key_with_error (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test = {0};
	uint16_t requestor_id = 0;
	uint8_t key_out[4096];
...
static void ephemeral_key_manager_test_execute (CuTest *test)
{
	...
	/* Mock setup */
	status = mock_expect (&manager_test.key_cache_mock.mock,
		manager_test.key_cache_mock.base.is_error_state, &manager_test.key_cache_mock, false);

	status |= mock_expect (&manager_test.key_cache_mock.mock,
		manager_test.key_cache_mock.base.is_initialized, &manager_test.key_cache_mock, true);
...
static void ephemeral_key_manager_test_execute_with_is_error_state_true (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test = {0};
	const platform_clock *next_time;
	int status = 0;
	...
	/* Mock setup */
	status = mock_expect (&manager_test.key_cache_mock.mock,
		manager_test.key_cache_mock.base.is_error_state, &manager_test.key_cache_mock, true);
	CuAssertIntEquals (test, 0, status);

	manager_test.key_manager.base.execute (&manager_test.key_manager.base);

	next_time = manager_test.key_manager.base.get_next_execution (&manager_test.key_manager.base);
	CuAssertPtrEquals (test, &manager_test.key_manager.state->next, (void*) next_time);

	ephemeral_key_manager_testing_release (test, &manager_test);
}

static void ephemeral_key_manager_test_execute_with_is_initialized_false (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test = {0};
	int status = 0;
	...
	status |= mock_expect (&manager_test.key_cache_mock.mock,
		manager_test.key_cache_mock.base.is_initialized, &manager_test.key_cache_mock, false);

	status |= mock_expect (&manager_test.key_cache_mock.mock,
		manager_test.key_cache_mock.base.initialize_cache, &manager_test.key_cache_mock, 0);
	CuAssertIntEquals (test, 0, status);
...
static void ephemeral_key_manager_test_execute_with_initialize_cache_failed (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test = {0};
	int status = 0;
	struct debug_log_entry_info entry = {
		...
		.msg_index = KEYSTORE_LOGGING_CACHE_INIT_FAIL,
		.arg1 = KEY_CACHE_INVALID_ARGUMENT,
		.arg2 = 0
	};
	...
	status |= mock_expect (&manager_test.key_cache_mock.mock,
		manager_test.key_cache_mock.base.initialize_cache, &manager_test.key_cache_mock,
		KEY_CACHE_INVALID_ARGUMENT);
...
static void ephemeral_key_manager_test_execute_with_is_initialized_true (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test = {0};
	int status = 0;
...
static void ephemeral_key_manager_test_execute_with_generate_key_error_response (CuTest *test)
{
	...
	struct debug_log_entry_info entry = {
		...
		.severity = DEBUG_LOG_SEVERITY_ERROR,
		.component = DEBUG_LOG_COMPONENT_KEYSTORE,
		.msg_index = KEYSTORE_LOGGING_EPHEMERAL_KEY_GENERATION_FAIL,
...
static void ephemeral_key_manager_test_execute_with_add_failed (CuTest *test)
{
	...
	struct debug_log_entry_info entry = {
		...
		.severity = DEBUG_LOG_SEVERITY_ERROR,
		.component = DEBUG_LOG_COMPONENT_KEYSTORE,
		.msg_index = KEYSTORE_LOGGING_EPHEMERAL_KEY_ADD_FAIL,
...
static void ephemeral_key_manager_test_execute_with_is_initialized_false_static_init (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test = {
		.key_manager = ephemeral_key_manager_static_init (&manager_test.state,
			&manager_test.key_cache_mock.base, &manager_test.key_gen_mock.base,
...
static void ephemeral_key_manager_test_execute_with_is_initialized_true_static_init (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test = {
		.key_manager = ephemeral_key_manager_static_init (&manager_test.state,
			&manager_test.key_cache_mock.base, &manager_test.key_gen_mock.base,
...
static void ephemeral_key_manager_test_get_key_size (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test;
	int status;
	...
	status = ephemeral_key_manager_get_key_size (&manager_test.key_manager);
	CuAssertIntEquals (test, EPHEMERAL_KEY_MANAGER_RSA_2K_KEY_SIZE, status);

	ephemeral_key_manager_testing_release (test, &manager_test);
}

static void ephemeral_key_manager_test_get_key_size_static_init (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test = {
		.key_manager = ephemeral_key_manager_static_init (&manager_test.state,
			&manager_test.key_cache_mock.base, &manager_test.key_gen_mock.base,
	...
	status = ephemeral_key_manager_get_key_size (&manager_test.key_manager);
	CuAssertIntEquals (test, EPHEMERAL_KEY_MANAGER_RSA_4K_KEY_SIZE, status);

	ephemeral_key_manager_testing_release (test, &manager_test);
}

static void ephemeral_key_manager_test_get_key_size_with_null (CuTest *test)
{
	struct ephemeral_key_manager_testing manager_test;
	int status;
	...
	status = ephemeral_key_manager_get_key_size (NULL);
	CuAssertIntEquals (test, EPHEMERAL_KEY_MANAGER_INVALID_ARGUMENT, status);
...
TEST (ephemeral_key_manager_test_get_key_null);
TEST (ephemeral_key_manager_test_get_key_with_error);
...
TEST (ephemeral_key_manager_test_execute_with_is_error_state_true);
TEST (ephemeral_key_manager_test_execute_with_is_initialized_false);
TEST (ephemeral_key_manager_test_execute_with_is_initialized_true);
TEST (ephemeral_key_manager_test_execute_with_initialize_cache_failed);
...
TEST (ephemeral_key_manager_test_execute_with_is_initialized_false_static_init);
TEST (ephemeral_key_manager_test_execute_with_is_initialized_true_static_init);
TEST (ephemeral_key_manager_test_get_key_size);
TEST (ephemeral_key_manager_test_get_key_size_static_init);
TEST (ephemeral_key_manager_test_get_key_size_with_null);
