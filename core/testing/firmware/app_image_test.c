...
static void app_image_test_get_hash_null (CuTest *test)
{
	...
	status = app_image_get_hash (NULL, 0x10000, &hash.base, hash_actual, sizeof (hash_actual));
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);

	status = app_image_get_hash (&flash.base, 0x10000, NULL, hash_actual, sizeof (hash_actual));
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);

	status = app_image_get_hash (&flash.base, 0x10000, &hash.base, NULL, sizeof (hash_actual));
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);
...
static void app_image_test_verification_null (CuTest *test)
{
	...
	status = app_image_verification (NULL, 0x20000, &hash.base, &rsa.base, &RSA_PUBLIC_KEY,	NULL,
		0);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);

	status = app_image_verification (&flash.base, 0x20000, NULL, &rsa.base, &RSA_PUBLIC_KEY, NULL,
		0);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);

	status = app_image_verification (&flash.base, 0x20000, &hash.base, NULL, &RSA_PUBLIC_KEY, NULL,
		0);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);

	status = app_image_verification (&flash.base, 0x20000, &hash.base, &rsa.base, NULL,	NULL, 0);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);
...
static void app_image_test_load_and_verify_null (CuTest *test)
{
	...
	status = app_image_load_and_verify (NULL, 0x20000, load_data, sizeof (load_data), &hash.base,
		&rsa.base, &RSA_PUBLIC_KEY, hash_out, sizeof (hash_out), &app_len);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);

	status = app_image_load_and_verify (&flash.base, 0x20000, NULL, sizeof (load_data),	&hash.base,
		&rsa.base, &RSA_PUBLIC_KEY, hash_out, sizeof (hash_out), &app_len);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);

	status = app_image_load_and_verify (&flash.base, 0x20000, load_data, sizeof (load_data), NULL,
		&rsa.base, &RSA_PUBLIC_KEY, hash_out, sizeof (hash_out), &app_len);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);
...
static void app_image_test_load_and_verify_update_hash_image_error (CuTest *test)
{
	...
	status |= mock_expect (&hash.mock, hash.base.start_sha256, &hash, 0);
	status |= mock_expect (&hash.mock, hash.base.update, &hash, 0, MOCK_ARG_NOT_NULL, MOCK_ARG (4));
	status |= mock_expect (&hash.mock, hash.base.update, &hash, HASH_ENGINE_UPDATE_FAILED,
...
static void app_image_test_verification_with_header_null (CuTest *test)
{
	...
	status = app_image_verification_with_header (NULL, 0x20000, APP_IMAGE_HEADER_LENGTH, &hash.base,
		&rsa.base, &RSA_PUBLIC_KEY, NULL, 0);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);
...
static void app_image_test_get_hash_with_header_null (CuTest *test)
{
	...
	status = app_image_get_hash_with_header (NULL, 0x10000, APP_IMAGE_HEADER_LENGTH, &hash.base,
		hash_actual, sizeof (hash_actual));
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);

	status = app_image_get_hash_with_header (&flash.base, 0x10000, APP_IMAGE_HEADER_LENGTH,	NULL,
		hash_actual, sizeof (hash_actual));
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);
...
static void app_image_test_load_and_verify_with_header (CuTest *test)
{
	...
	status |= mock_expect (&flash.mock, flash.base.read, &flash, 0,
		MOCK_ARG (0x20004 + APP_IMAGE_HEADER_LENGTH), MOCK_ARG_NOT_NULL,
	...
	status = testing_validate_array (APP_IMAGE_HEADER_DATA + 4 + APP_IMAGE_HEADER_LENGTH, load_data,
		APP_IMAGE_HEADER_DATA_LENGTH - 4 - APP_IMAGE_HEADER_LENGTH);
	CuAssertIntEquals (test, 0, status);
...
static void app_image_test_load_and_verify_with_header_null (CuTest *test)
{
	...
	status = app_image_load_and_verify_with_header (&flash.base, 0x20000, APP_IMAGE_HEADER_LENGTH,
		load_data, sizeof (load_data), &hash.base, &rsa.base, NULL, hash_out, sizeof (hash_out),
		&app_len);
	CuAssertIntEquals (test, APP_IMAGE_INVALID_ARGUMENT, status);
