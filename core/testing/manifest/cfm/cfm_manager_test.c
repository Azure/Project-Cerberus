...
static void cfm_manager_test_on_cfm_verified_no_observers (CuTest *test)
{
	...
	status = mock_expect (&manager.mock, manager.base.get_pending_cfm, &manager,
		MOCK_RETURN_PTR (&cfm));
...
static void cfm_manager_test_on_cfm_activation_request_no_observers (CuTest *test)
{
	struct cfm_manager_mock manager;
	int status;
	...
	cfm_manager_on_cfm_activation_request (&manager.base);

	status = cfm_manager_mock_validate_and_release (&manager);
	CuAssertIntEquals (test, 0, status);
}

static void cfm_manager_test_on_cfm_activation_request_one_observer (CuTest *test)
{
	struct cfm_manager_mock manager;
	struct cfm_observer_mock observer;
	int status;
	...
	status = mock_expect (&observer.mock, observer.base.on_cfm_activation_request, &observer, 0);
	CuAssertIntEquals (test, 0, status);
...
static void cfm_manager_test_on_cfm_activation_request_no_event_handler (CuTest *test)
{
	struct cfm_manager_mock manager;
	struct cfm_observer_mock observer;
	int status;
	...
	observer.base.on_cfm_activation_request = NULL;
...
static void cfm_manager_test_on_cfm_activation_request_null (CuTest *test)
{
	TEST_START;

	cfm_manager_on_cfm_activation_request (NULL);
}
...
static void cfm_manager_test_get_platform_id_measured_data (CuTest *test)
{
	...
	status |= mock_expect (&cfm.mock, cfm.base.base.free_platform_id, &cfm, 0, MOCK_ARG_PTR (id));
...
static void cfm_manager_test_get_cfm_measured_data_null (CuTest *test)
{
	...
	status = cfm_manager_get_cfm_measured_data (&manager.base, 0, NULL, length, &total_len);
	CuAssertIntEquals (test, MANIFEST_MANAGER_INVALID_ARGUMENT, status);
...
static void cfm_manager_test_hash_id_measured_data (CuTest *test)
{
	...
	status = cfm_manager_hash_id_measured_data (&manager.base, &hash.base);
	CuAssertIntEquals (test, 0, status);
...
static void cfm_manager_test_hash_id_measured_data_no_active_cfm (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_mock cfm;
	struct cfm_manager_mock manager;
...
static void cfm_manager_test_hash_id_measured_data_null (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_manager_mock manager;
	int status;
	...
	status = cfm_manager_hash_id_measured_data (NULL, &hash.base);
	CuAssertIntEquals (test, MANIFEST_MANAGER_INVALID_ARGUMENT, status);
	...
	status = cfm_manager_hash_id_measured_data (&manager.base, NULL);
	CuAssertIntEquals (test, MANIFEST_MANAGER_INVALID_ARGUMENT, status);
...
static void cfm_manager_test_hash_id_measured_data_fail (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_mock cfm;
	struct cfm_manager_mock manager;
...
static void cfm_manager_test_hash_platform_id_measured_data (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_mock cfm;
	struct cfm_manager_mock manager;
	const char *id = "CFM Test1";
	int status;
	...
	status = cfm_manager_hash_platform_id_measured_data (&manager.base, &hash.base);
	CuAssertIntEquals (test, 0, status);
...
static void cfm_manager_test_hash_platform_id_measured_data_no_active_cfm (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_mock cfm;
	struct cfm_manager_mock manager;
...
static void cfm_manager_test_hash_platform_id_measured_data_null (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_manager_mock manager;
	int status;
	...
	status = cfm_manager_hash_platform_id_measured_data (NULL, &hash.base);
	CuAssertIntEquals (test, MANIFEST_MANAGER_INVALID_ARGUMENT, status);
	...
	status = cfm_manager_hash_platform_id_measured_data (&manager.base, NULL);
	CuAssertIntEquals (test, MANIFEST_MANAGER_INVALID_ARGUMENT, status);
...
static void cfm_manager_test_hash_platform_id_measured_data_fail (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_mock cfm;
	struct cfm_manager_mock manager;
...
static void cfm_manager_test_hash_cfm_measured_data (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_mock cfm;
	struct cfm_manager_mock manager;
	...
	status |= mock_expect (&hash.mock, hash.base.update, &hash, 0,
		MOCK_ARG_PTR_CONTAINS (CFM_TESTING.manifest.hash, CFM_TESTING.manifest.hash_len),
		MOCK_ARG (CFM_TESTING.manifest.hash_len));

	CuAssertIntEquals (test, 0, status);

	status = cfm_manager_hash_cfm_measured_data (&manager.base, &hash.base);
	CuAssertIntEquals (test, 0, status);
...
static void cfm_manager_test_hash_cfm_measured_data_no_active_cfm (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_mock cfm;
	struct cfm_manager_mock manager;
...
static void cfm_manager_test_hash_cfm_measured_data_null (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_manager_mock manager;
	int status;
	...
	status = cfm_manager_hash_cfm_measured_data (NULL, &hash.base);
	CuAssertIntEquals (test, MANIFEST_MANAGER_INVALID_ARGUMENT, status);
	...
	status = cfm_manager_hash_cfm_measured_data (&manager.base, NULL);
	CuAssertIntEquals (test, MANIFEST_MANAGER_INVALID_ARGUMENT, status);
...
static void cfm_manager_test_hash_cfm_measured_data_fail (CuTest *test)
{
	struct hash_engine_mock hash;
	struct cfm_mock cfm;
	struct cfm_manager_mock manager;
...
TEST (cfm_manager_test_on_cfm_activation_request_no_observers);
TEST (cfm_manager_test_on_cfm_activation_request_one_observer);
TEST (cfm_manager_test_on_cfm_activation_request_no_event_handler);
TEST (cfm_manager_test_on_cfm_activation_request_null);
...
TEST (cfm_manager_test_hash_id_measured_data);
TEST (cfm_manager_test_hash_id_measured_data_no_active_cfm);
TEST (cfm_manager_test_hash_id_measured_data_null);
TEST (cfm_manager_test_hash_id_measured_data_fail);
TEST (cfm_manager_test_hash_platform_id_measured_data);
TEST (cfm_manager_test_hash_platform_id_measured_data_no_active_cfm);
TEST (cfm_manager_test_hash_platform_id_measured_data_null);
TEST (cfm_manager_test_hash_platform_id_measured_data_fail);
TEST (cfm_manager_test_hash_cfm_measured_data);
TEST (cfm_manager_test_hash_cfm_measured_data_no_active_cfm);
TEST (cfm_manager_test_hash_cfm_measured_data_null);
