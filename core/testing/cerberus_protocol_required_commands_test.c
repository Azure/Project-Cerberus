...
static const char *SUITE = "cerberus_protocol_required_commands";
	...
	struct session_manager_mock *session)
{
	...
	req->digest.slot_num = 0;
	req->digest.key_alg = 1;
	...
	CuTest *test, struct cmd_interface *cmd)
{
	...
	req->digest.slot_num = 0;
	req->digest.key_alg = NUM_ATTESTATION_KEY_EXCHANGE_ALGORITHMS;
	request.length = sizeof (struct cerberus_protocol_get_certificate_digest);
	request.max_response = MCTP_PROTOCOL_MAX_MESSAGE_BODY;
	request.source_eid = MCTP_PROTOCOL_BMC_EID;
	request.target_eid = MCTP_PROTOCOL_PA_ROT_CTRL_EID;
	...
	struct cmd_interface *cmd, struct attestation_slave_mock *slave_attestation)
{
	...
	req->certificate.slot_num = 0;
	req->certificate.cert_num = 0;
	req->certificate.offset = 0;
	req->certificate.length = 10;
	...
	struct cmd_interface *cmd, struct attestation_slave_mock *slave_attestation)
{
	...
	req->certificate.slot_num = 1;
	req->certificate.cert_num = 1;
	req->certificate.offset = 0;
	req->certificate.length = 0;
	request.length = sizeof (struct cerberus_protocol_get_certificate);
	request.max_response = MCTP_PROTOCOL_MAX_MESSAGE_BODY;
	...
	CuTest *test, struct cmd_interface *cmd, struct attestation_slave_mock *slave_attestation)
{
	...
	req->certificate.slot_num = 0;
	req->certificate.cert_num = 0;
	req->certificate.offset = X509_CERTCA_ECC_CA_NOPL_DER_LEN + 1;
	req->certificate.length = 0;
	request.length = sizeof (struct cerberus_protocol_get_certificate);
	request.max_response = MCTP_PROTOCOL_MAX_MESSAGE_BODY;
	request.source_eid = MCTP_PROTOCOL_BMC_EID;
	...
	CuTest *test, struct cmd_interface *cmd, struct attestation_slave_mock *slave_attestation)
{
	...
	req->certificate.slot_num = 0;
	req->certificate.cert_num = 0;
	req->certificate.offset = X509_CERTCA_ECC_CA_NOPL_DER_LEN - 2;
	req->certificate.length = X509_CERTCA_ECC_CA_NOPL_DER_LEN;
	request.length = sizeof (struct cerberus_protocol_get_certificate);
	request.max_response = MCTP_PROTOCOL_MAX_MESSAGE_BODY;
	request.source_eid = MCTP_PROTOCOL_BMC_EID;
	request.target_eid = MCTP_PROTOCOL_PA_ROT_CTRL_EID;
	...
	struct session_manager_mock *session)
{
	...
	digest_req->digest.slot_num = 0;
	digest_req->digest.key_alg = ATTESTATION_ECDHE_KEY_EXCHANGE;
	digest_request.length = sizeof (struct cerberus_protocol_get_certificate_digest);
	digest_request.max_response = MCTP_PROTOCOL_MAX_MESSAGE_BODY;
	digest_request.source_eid = MCTP_PROTOCOL_BMC_EID;
	digest_request.target_eid = MCTP_PROTOCOL_PA_ROT_CTRL_EID;
	...
	digest_request.new_request = true;
	status = cmd->process_request (cmd, &digest_request);
	CuAssertIntEquals (test, 0, status);
	CuAssertIntEquals (test, false, digest_request.new_request);
	...
	status = mock_expect (&session->mock, session->base.add_session, session, 0,
		MOCK_ARG (MCTP_PROTOCOL_BMC_EID), MOCK_ARG_PTR_CONTAINS_TMP (nonce, sizeof (nonce)),
	...
	CuTest *test, struct cmd_interface *cmd, struct attestation_slave_mock *slave_attestation)
{
	...
	digest_req->digest.slot_num = 0;
	digest_req->digest.key_alg = ATTESTATION_KEY_EXCHANGE_NONE;
	digest_request.length = sizeof (struct cerberus_protocol_get_certificate_digest);
	digest_request.max_response = MCTP_PROTOCOL_MAX_MESSAGE_BODY;
	digest_request.source_eid = MCTP_PROTOCOL_BMC_EID;
	digest_request.target_eid = MCTP_PROTOCOL_PA_ROT_CTRL_EID;
	...
	struct cmd_interface *cmd, struct device_manager *device_manager)
{
	...
	expected_out.request.max_message_size = MCTP_PROTOCOL_MAX_MESSAGE_BODY;
	expected_out.request.max_packet_size = MCTP_PROTOCOL_MAX_TRANSMISSION_UNIT;
	...
	expected_out.max_timeout = MCTP_PROTOCOL_MAX_RESPONSE_TIMEOUT_MS / 10;
	...
	struct cmd_interface *cmd, struct session_manager_mock *session)
{
	...
	encrypted_error_packet.source_eid = MCTP_PROTOCOL_BMC_EID;
	...
	status = mock_expect (&session->mock, session->base.reset_session, session,
		SESSION_MANAGER_NO_MEMORY, MOCK_ARG (MCTP_PROTOCOL_BMC_EID),
...
static void cerberus_protocol_required_commands_test_get_digest_format (CuTest *test)
{
	...
	CuAssertIntEquals (test, 0x02, req->digest.key_alg);
...
static void cerberus_protocol_required_commands_test_get_certificate_format (CuTest *test)
{
	...
	CuAssertIntEquals (test, 0x01, req->certificate.cert_num);
	CuAssertIntEquals (test, 0x0302, req->certificate.offset);
	CuAssertIntEquals (test, 0x0504, req->certificate.length);
...
CuSuite* get_cerberus_protocol_required_commands_suite ()
{
	CuSuite *suite = CuSuiteNew ();

	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_header_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_error_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_device_capabilities_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_get_digest_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_get_certificate_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_challenge_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_import_certificate_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_export_csr_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_get_certificate_state_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_get_device_info_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_get_fw_version_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_get_device_id_format);
	SUITE_ADD_TEST (suite, cerberus_protocol_required_commands_test_reset_counter_format);
