...
static void setup_session_manager_ecc_test (CuTest *test, struct session_manager_ecc_testing *cmd)
{
	...
	status = aes_gcm_mock_init (&cmd->aes);
...
static void release_session_manager_ecc_test (CuTest *test, struct session_manager_ecc_testing *cmd)
{
	...
	status = aes_gcm_mock_validate_and_release (&cmd->aes);
	...
	struct session_manager_ecc_testing *cmd, uint8_t eid)
{
	...
	uint8_t nonce1[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0xe4, 0x02, 0x34,
		0xd6, 0x41, 0x80, 0xfa, 0x1a, 0x0e, 0x0a, 0x04,
		0x0e, 0x9a, 0x37, 0xff, 0x3e, 0xa0, 0x05, 0x75,
		0x73, 0xc5, 0x54, 0x10, 0xad, 0xd5, 0xc5, 0xc6
	};
	uint8_t nonce2[] = {
		0x0e, 0x9a, 0x37, 0xff, 0x3e, 0xa0, 0x02, 0x75,
		0x73, 0xc5, 0x54, 0x10, 0xad, 0xd5, 0xc5, 0xc6,
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0xe4, 0x05, 0x34,
		0xd6, 0x41, 0x80, 0xfa, 0x1a, 0x0e, 0x0a, 0x04
	};
	uint8_t session_key[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0xe4, 0x05, 0x75,
		0x73, 0xc5, 0x54, 0x10, 0xad, 0xd5, 0xc5, 0xc6,
		0x0e, 0x9a, 0x37, 0xff, 0x3e, 0xa0, 0x02, 0x34,
		0xd6, 0x41, 0x80, 0xfa, 0x1a, 0x0e, 0x0a, 0x04
	};
	uint8_t digest[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0x0e, 0x9a, 0x37,
		0xe4, 0x05, 0x75, 0x73, 0xc5, 0x54, 0x10, 0xad,
		0xff, 0x3e, 0xa0, 0x02, 0x34, 0xd6, 0x41, 0x80,
		0xfa, 0x1a, 0x0e, 0x0a, 0x04, 0xd5, 0xc5, 0xc6
	};
	uint8_t hmac[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0xe4, 0x05, 0xad,
		0xd5, 0xc5, 0xc6, 0x9a, 0x37, 0xff, 0x3e, 0x75,
		0x0e, 0x73, 0xc5, 0x54, 0x10, 0xa0, 0x02, 0x34,
		0xd6, 0x41, 0x80, 0xfa, 0x1a, 0x0e, 0x0a, 0x04
	};
	...
	status |= mock_expect (&cmd->hash.mock, cmd->hash.base.get_active_algorithm, &cmd->hash,
		HASH_TYPE_SHA256);
	...
	status |= mock_expect (&cmd->ecc.mock, cmd->ecc.base.sign, &cmd->ecc, ECC_SIG_TEST_LEN,
		...
		MOCK_ARG (sizeof (digest)), MOCK_ARG_PTR (NULL), MOCK_ARG_NOT_NULL,
		MOCK_ARG (rq.max_response - sizeof (struct cerberus_protocol_key_exchange_response_type_0) -
		ECC_PUBKEY2_DER_LEN -
		sizeof (uint16_t)));
	status |= mock_expect_output (&cmd->ecc.mock, 4, ECC_SIGNATURE_TEST, ECC_SIG_TEST_LEN, 5);
...
static void session_manager_ecc_test_establish_session (CuTest *test)
{
	...
	uint8_t hmac_key[] = {
		0xf1, 0x3b, 0x43, 0x16, 0xd5, 0xc5, 0xc6, 0x10,
		0xad, 0xff, 0x3e, 0xa0, 0x02, 0x34, 0xd6, 0x37,
		0x0e, 0x9a, 0x2c, 0xe4, 0x05, 0x75, 0x73, 0xc5,
		0x54, 0x41, 0x80, 0xfa, 0x1a, 0x0e, 0x0a, 0x04
	};
	uint8_t digest[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0x0e, 0x9a, 0x37,
		0xe4, 0x05, 0x75, 0x73, 0xc5, 0x54, 0x10, 0xad,
...
static void session_manager_ecc_test_decrypt_message_set_key_fail (CuTest *test)
{
	...
	status = mock_expect (&cmd.aes.mock, cmd.aes.base.set_key, &cmd.aes, AES_GCM_ENGINE_NO_MEMORY,
		MOCK_ARG_PTR_CONTAINS_TMP (aes_key, sizeof (aes_key)), MOCK_ARG (sizeof (aes_key)));
	CuAssertIntEquals (test, 0, status);

	status = cmd.session.base.decrypt_message (&cmd.session.base, &rq);
	CuAssertIntEquals (test, AES_GCM_ENGINE_NO_MEMORY, status);
...
static void session_manager_ecc_test_decrypt_message_fail (CuTest *test)
{
	...
	status = mock_expect (&cmd.aes.mock, cmd.aes.base.decrypt_data, &cmd.aes,
		AES_GCM_ENGINE_NO_MEMORY, MOCK_ARG_PTR_CONTAINS (rq.data +
...
static void session_manager_ecc_test_get_pairing_state (CuTest *test)
{
	...
	uint8_t pairing_key[] = {
		0xf1, 0x3b, 0x43, 0x16, 0xc6, 0x10, 0x34, 0xd6,
		0x37, 0xff, 0x3e, 0xa0, 0x02, 0x73, 0xc5, 0x54,
		0x0e, 0x9a, 0x2c, 0xe4, 0x05, 0x75, 0xd5, 0xc5,
		0xad, 0x80, 0xfa, 0x1a, 0x0e, 0x0a, 0x04, 0x41
	};
	uint8_t hmac[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0xe4, 0x05, 0x75,
		0x0e, 0x9a, 0x37, 0xff, 0x3e, 0xa0, 0x02, 0x34,
...
static void session_manager_ecc_test_reset_session_with_hmac_not_permitted (CuTest *test)
{
	...
	uint8_t hmac2[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0xe4, 0x05, 0x75,
		0x0e, 0x9a, 0x37, 0xff, 0x3e, 0xa0, 0x02, 0x34,
		0xaa, 0x41, 0x80, 0xfa, 0x1a, 0x0e, 0x0a, 0x04,
		0x73, 0xc5, 0x54, 0x10, 0xad, 0xd5, 0xc5, 0xc6
	};
	int status;
...
static void session_manager_ecc_test_setup_paired_session_already_paired (CuTest *test)
{
	...
	uint8_t hmac2[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0xe4, 0x05, 0x75,
		0x0e, 0x9a, 0x37, 0xff, 0x3e, 0xa0, 0x02, 0x34,
		0x0a, 0x04, 0x73, 0xc5, 0x54, 0x10, 0xad, 0xd5,
		0xc5, 0xc6, 0xd6, 0x41, 0x80, 0xfa, 0x1a, 0x0e
	};
	uint8_t *pairing_key_buf;
	size_t pairing_key_buf_len = sizeof (pairing_key);
	uint8_t separator = 0;
...
static void session_manager_ecc_test_session_sync (CuTest *test)
{
	...
	uint8_t hmac_expected[] = {
		0xf1, 0x3b, 0x43, 0x16, 0x2c, 0xe4, 0x05, 0x75,
		0x0e, 0x9a, 0x37, 0xff, 0x3e, 0xa0, 0x02, 0x34,
		0xd6, 0x41, 0x20, 0xfa, 0x1a, 0x0e, 0x0a, 0x04,
		0x73, 0xc5, 0x54, 0x10, 0xad, 0xd5, 0xc5, 0xc6
	};
