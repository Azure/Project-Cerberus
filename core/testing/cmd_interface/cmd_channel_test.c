...
static void setup_mock_cmd_channel_test (CuTest *test, struct cmd_channel_testing *channel)
{
	...
	status = cmd_interface_multi_handler_mock_init (&channel->req_handler);
	CuAssertIntEquals (test, 0, status);
	...
	status = mctp_interface_init (&channel->mctp, &channel->mctp_state, &channel->req_handler.base,
		&channel->device_mgr, &channel->test.base, &channel->cmd_cerberus.base, NULL, NULL);
	CuAssertIntEquals (test, 0, status);
}
...
static void complete_mock_cmd_channel_test (CuTest *test, struct cmd_channel_testing *channel)
{
	...
	status |= cmd_interface_multi_handler_mock_validate_and_release (&channel->req_handler);
...
static void cmd_channel_test_receive_and_process_null (CuTest *test)
{
	...
	struct cmd_interface_msg request;
	struct cmd_interface_msg response;
	uint8_t response_data[sizeof (struct cerberus_protocol_error)];
	struct mctp_base_protocol_transport_header *header =
		(struct mctp_base_protocol_transport_header*) rx_packet[0].data;
	struct cerberus_protocol_error *error = (struct cerberus_protocol_error*) response_data;
	uint16_t pci_vid = 0x1414;
	uint8_t payload[msg_size];
	int i;
...
static void cmd_channel_test_receive_and_process_multiple_overflow_packet (CuTest *test)
{
	...
	status = mock_expect (&channel.test.mock, channel.test.base.receive_packet, &channel.test, 0,
		MOCK_ARG_NOT_NULL, MOCK_ARG (-1));
	status |= mock_expect_output (&channel.test.mock, 0, &rx_packet[3],	sizeof (struct cmd_packet),
		-1);
