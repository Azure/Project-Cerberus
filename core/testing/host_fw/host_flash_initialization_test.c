...
static void host_flash_initialization_test_init (CuTest *test)
{
	...
	struct host_flash_initialization_state state;
	...
	status = host_flash_initialization_init (&init, &state, &flash0, &state0, &flash_mock0.base,
		&flash1, &state1, &flash_mock1.base, false, false);
	CuAssertIntEquals (test, 0, status);
...
static void host_flash_initialization_test_init_null (CuTest *test)
{
	...
	status = host_flash_initialization_init (NULL, &state, &flash0, &state0, &flash_mock0.base,
		&flash1, &state1, &flash_mock1.base, false, false);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init (&init, NULL, &flash0, &state0, &flash_mock0.base,
		&flash1, &state1, &flash_mock1.base, false, false);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init (&init, &state, NULL, &state0, &flash_mock0.base,
		&flash1, &state1, &flash_mock1.base, false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init (&init, &state, &flash0, NULL, &flash_mock0.base,
		&flash1, &state1, &flash_mock1.base, false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init (&init, &state, &flash0, &state0, NULL, &flash1,
		&state1, &flash_mock1.base, false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init (&init, &state, &flash0, &state0, &flash_mock0.base,
		NULL, &state1, &flash_mock1.base, false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init (&init, &state, &flash0, &state0, &flash_mock0.base,
		&flash1, NULL, &flash_mock1.base, false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init (&init, &state, &flash0, &state0, &flash_mock0.base,
		&flash1, &state1, NULL, false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);
...
static void host_flash_initialization_test_init_single_flash (CuTest *test)
{
	...
	status = host_flash_initialization_init_single_flash (&init, &state, &flash, &flash_state,
		&flash_mock.base, false, false);
	CuAssertIntEquals (test, 0, status);
...
static void host_flash_initialization_test_init_single_flash_null (CuTest *test)
{
	...
	status = host_flash_initialization_init_single_flash (NULL, &state, &flash, &flash_state,
		&flash_mock.base, false, false);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init_single_flash (&init, NULL, &flash, &flash_state,
		&flash_mock.base, false, false);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init_single_flash (&init, &state, NULL, &flash_state,
		&flash_mock.base, false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init_single_flash (&init, &state, &flash, NULL,
		&flash_mock.base, false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init_single_flash (&init, &state, &flash, &flash_state, NULL,
		false, false);
	CuAssertIntEquals (test, SPI_FLASH_INVALID_ARGUMENT, status);
...
static void host_flash_initialization_test_static_init (CuTest *test)
{
	struct flash_master_mock flash_mock0;
	struct flash_master_mock flash_mock1;
	struct spi_flash_state state0;
	...
	struct spi_flash flash1;
	struct host_flash_initialization_state state;
	struct host_flash_initialization init = host_flash_initialization_static_init (&state, &flash0,
		&flash1, false, false);
	int status;
	...
	status = spi_flash_init_api (&flash0, &state0, &flash_mock0.base);
	CuAssertIntEquals (test, 0, status);

	status = spi_flash_init_api (&flash1, &state1, &flash_mock1.base);
	CuAssertIntEquals (test, 0, status);

	status = host_flash_initialization_init_state (&init);
	CuAssertIntEquals (test, 0, status);
...
static void host_flash_initialization_test_static_init_null (CuTest *test)
{
	struct flash_master_mock flash_mock0;
	struct flash_master_mock flash_mock1;
	struct spi_flash_state state0;
	...
	struct spi_flash flash1;
	struct host_flash_initialization_state state;
	struct host_flash_initialization null_state = host_flash_initialization_static_init (NULL,
		&flash0, &flash1, false, false);
	struct host_flash_initialization null_flash0 = host_flash_initialization_static_init (&state,
		NULL, &flash1, false, false);
	struct host_flash_initialization null_flash1 = host_flash_initialization_static_init (&state,
		&flash0, NULL, false, false);
	int status;
	...
	status = host_flash_initialization_init_state (NULL);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init_state (&null_state);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init_state (&null_flash0);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);

	status = host_flash_initialization_init_state (&null_flash1);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);
...
static void host_flash_initialization_test_static_init_single_flash (CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
	struct spi_flash flash;
	struct host_flash_initialization_state state;
	struct host_flash_initialization init =
		host_flash_initialization_static_init_single_flash (&state, &flash, false, false);
	int status;
	...
	status = spi_flash_init_api (&flash, &flash_state, &flash_mock.base);
	CuAssertIntEquals (test, 0, status);
...
static void host_flash_initialization_test_static_init_single_flash_null (CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
	struct spi_flash flash;
	struct host_flash_initialization_state state;
	struct host_flash_initialization null_state =
		host_flash_initialization_static_init_single_flash (NULL, &flash, false, false);
	struct host_flash_initialization null_flash =
		host_flash_initialization_static_init_single_flash (&state, NULL, false, false);
	int status;
	...
	status = host_flash_initialization_init_state (&null_flash);
	CuAssertIntEquals (test, HOST_FLASH_INIT_INVALID_ARGUMENT, status);
...
static void host_flash_initialization_test_initialize_flash_fast_read (CuTest *test)
{
	...
	status = host_flash_initialization_init (&init, &state, &flash0, &state0, &flash_mock0.base,
		&flash1, &state1, &flash_mock1.base, true, false);
	CuAssertIntEquals (test, 0, status);
...
static void host_flash_initialization_test_initialize_flash_drive_strength (CuTest *test)
{
	...
	status = host_flash_initialization_init (&init, &state, &flash0, &state0, &flash_mock0.base,
		&flash1, &state1, &flash_mock1.base, false, true);
	CuAssertIntEquals (test, 0, status);
...
static void host_flash_initialization_test_initialize_flash_single_flash_fast_read (CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
	struct spi_flash flash;
...
static void host_flash_initialization_test_initialize_flash_single_flash_drive_strength (
	CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
	...
	status = host_flash_initialization_init_single_flash (&init, &state, &flash, &flash_state,
		&flash_mock.base, false, true);
	CuAssertIntEquals (test, 0, status);

	/* Get Device ID. */
	status = flash_master_mock_expect_rx_xfer (&flash_mock, 0, winbond, id_length,
		FLASH_EXP_READ_REG (0x9f, id_length));
	...
	/* Configure output drive strength. */
	status |= flash_master_mock_expect_rx_xfer (&flash_mock, 0, &initial, sizeof (initial),
		FLASH_EXP_READ_REG (0x15, sizeof (initial)));
	...
	status |= flash_master_mock_expect_rx_xfer (&flash_mock, 0, &configured, sizeof (configured),
		FLASH_EXP_READ_REG (0x15, sizeof (configured)));
...
static void host_flash_initialization_test_initialize_flash_single_flash_twice (CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
	struct spi_flash flash;
...
static void host_flash_initialization_test_initialize_flash_static_init (CuTest *test)
{
	struct flash_master_mock flash_mock0;
	struct flash_master_mock flash_mock1;
	struct spi_flash_state state0;
	struct spi_flash_state state1;
	struct spi_flash flash0 = spi_flash_static_init (SPI_FLASH_API_INIT, &state0,
		&flash_mock0.base);
	struct spi_flash flash1 = spi_flash_static_init (SPI_FLASH_API_INIT, &state1,
		&flash_mock1.base);
	struct host_flash_initialization_state state;
	struct host_flash_initialization init = host_flash_initialization_static_init (&state, &flash0,
		&flash1, false, false);
...
static void host_flash_initialization_test_initialize_flash_static_init_fast_read (CuTest *test)
{
	struct flash_master_mock flash_mock0;
	struct flash_master_mock flash_mock1;
	struct spi_flash_state state0;
...
static void host_flash_initialization_test_initialize_flash_static_init_drive_strength (
	CuTest *test)
{
	struct flash_master_mock flash_mock0;
	struct flash_master_mock flash_mock1;
...
static void host_flash_initialization_test_initialize_flash_static_init_twice (CuTest *test)
{
	struct flash_master_mock flash_mock0;
	struct flash_master_mock flash_mock1;
	struct spi_flash_state state0;
...
static void host_flash_initialization_test_initialize_flash_static_init_single_flash (CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
	struct spi_flash flash = spi_flash_static_init (SPI_FLASH_API_INIT, &flash_state,
		&flash_mock.base);
	struct host_flash_initialization_state state;
	struct host_flash_initialization init =
		host_flash_initialization_static_init_single_flash (&state, &flash, false, false);
...
static void host_flash_initialization_test_initialize_flash_static_init_single_flash_fast_read (
	CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
	...
	struct host_flash_initialization_state state;
	struct host_flash_initialization init =
		host_flash_initialization_static_init_single_flash (&state, &flash, true, false);
	int status;
	uint32_t header[] = {
		0x50444653,
		0xff010106,
...
static void host_flash_initialization_test_initialize_flash_static_init_single_flash_drive_strength
	(CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
	...
	struct host_flash_initialization_state state;
	struct host_flash_initialization init =
		host_flash_initialization_static_init_single_flash (&state, &flash, false, true);
	int status;
	uint32_t header[] = {
		0x50444653,
		0xff010106,
...
static void host_flash_initialization_test_initialize_flash_static_init_single_flash_twice (
	CuTest *test)
{
	struct flash_master_mock flash_mock;
	struct spi_flash_state flash_state;
...
TEST (host_flash_initialization_test_static_init);
TEST (host_flash_initialization_test_static_init_null);
TEST (host_flash_initialization_test_static_init_single_flash);
TEST (host_flash_initialization_test_static_init_single_flash_null);
...
TEST (host_flash_initialization_test_initialize_flash_single_flash_fast_read);
TEST (host_flash_initialization_test_initialize_flash_single_flash_drive_strength);
TEST (host_flash_initialization_test_initialize_flash_single_flash_twice);
TEST (host_flash_initialization_test_initialize_flash_static_init);
TEST (host_flash_initialization_test_initialize_flash_static_init_fast_read);
TEST (host_flash_initialization_test_initialize_flash_static_init_drive_strength);
TEST (host_flash_initialization_test_initialize_flash_static_init_twice);
TEST (host_flash_initialization_test_initialize_flash_static_init_single_flash);
TEST (host_flash_initialization_test_initialize_flash_static_init_single_flash_fast_read);
TEST (host_flash_initialization_test_initialize_flash_static_init_single_flash_drive_strength);
TEST (host_flash_initialization_test_initialize_flash_static_init_single_flash_twice);
