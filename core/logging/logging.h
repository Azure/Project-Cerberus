 ...
 */
enum {
	LOGGING_INVALID_ARGUMENT = LOGGING_ERROR (0),			/**< Input parameter is null or not valid. */
	LOGGING_NO_MEMORY = LOGGING_ERROR (1),					/**< Memory allocation failed. */
	LOGGING_CREATE_ENTRY_FAILED = LOGGING_ERROR (2),		/**< A new entry was not added to the log. */
	LOGGING_FLUSH_FAILED = LOGGING_ERROR (3),				/**< The log was not stored to persistent memory. */
	LOGGING_CLEAR_FAILED = LOGGING_ERROR (4),				/**< The log entries were not cleared. */
	LOGGING_GET_SIZE_FAILED = LOGGING_ERROR (5),			/**< The log size could not be determined. */
	LOGGING_READ_CONTENTS_FAILED = LOGGING_ERROR (6),		/**< Could not retrieve the log entries. */
	LOGGING_INCOMPLETE_FLUSH = LOGGING_ERROR (7),			/**< Not all log entries were made accessible. */
	LOGGING_UNSUPPORTED_SEVERITY = LOGGING_ERROR (8),		/**< An entry was specified at an unsupported severity level. */
	LOGGING_STORAGE_NOT_ALIGNED = LOGGING_ERROR (9),		/**< Memory for the log is not aligned correctly. */
	LOGGING_BAD_ENTRY_LENGTH = LOGGING_ERROR (10),			/**< The entry data is not the right size for the log. */
	LOGGING_NO_LOG_AVAILABLE = LOGGING_ERROR (11),			/**< There is no log available for the operation. */
};
