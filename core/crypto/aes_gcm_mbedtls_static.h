...
#ifndef AES_GCM_MBEDTLS_STATIC_H_
#define AES_GCM_MBEDTLS_STATIC_H_

#include "crypto/aes_gcm_mbedtls.h"


/* Internal functions declared to allow for static initialization. */
int aes_gcm_mbedtls_set_key (const struct aes_gcm_engine *engine, const uint8_t *key,
	size_t length);
...
#define	AES_GCM_MBEDTLS_API_INIT { \
		...
		.clear_key = aes_gcm_mbedtls_clear_key, \
		.encrypt_data = aes_gcm_mbedtls_encrypt_data, \
		.encrypt_with_add_data = aes_gcm_mbedtls_encrypt_with_add_data, \
		.decrypt_data = aes_gcm_mbedtls_decrypt_data, \
		.decrypt_with_add_data = aes_gcm_mbedtls_decrypt_with_add_data, \
	}
...
#endif	/* AES_GCM_MBEDTLS_STATIC_H_ */
