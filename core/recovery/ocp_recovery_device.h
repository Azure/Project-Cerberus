...
/**
 * Interface to read/write data from/to a CMS region in flash.
 */
struct ocp_recovery_device_flash_cms {
	uint32_t base_addr;				/**< The base address for a fixed-length region of flash. */
	uint32_t *flash_erase_offset;	/**< Offset of erase address */
	const struct flash *flash;		/**< Flash device that contains the cms data. */
};
...
struct ocp_recovery_device_cms {
	union {
		...
		struct ocp_recovery_device_flash_cms *flash;				/**< Flash interface for the flash region. */
	};
	...
	bool cms_flash;						/**< Flag indicating if CMS access flash. */
};
...
struct ocp_recovery_device {
	...
	const struct ocp_recovery_device_cms *cms;	/**< Memory/Flash regions available for recovery commands. */
	size_t cms_count;							/**< Number of memory regions supported. */
};


/**
 * Initialize a static instance of a flash CMS wrapper.
 *
 * @param flash_ptr The flash device that contains the cms data.
 * @param flash_erase_offset The buffer to store the offset we already erase before write.
 * @param flash_addr The base address for a fixed-length region of flash.
 */
#define	ocp_recovery_device_flash_cms_static_init(flash_ptr, erase_offset, flash_addr)	{ \
	.flash = flash_ptr, \
	.flash_erase_offset = erase_offset, \
...
enum {
	...
	OCP_RECOVERY_DEVICE_CMS_FLASH_ADDR_OVERFLOW = OCP_RECOVERY_DEVICE_ERROR (0x12),	/**< Base address plus offset is greater than the device size. */
};
