...
void CuSuiteDetails(CuSuite* testSuite, CuString* details)
{
	...
	else
	{
		if (testSuite->failCount == 1)
			...
			platform_CuStringAppendFormat(details, "There were %d failures:" NEWLINE, testSuite->failCount);
		...
		for (i = 0 ; i < testSuite->count ; ++i)
		{
			...
			{
				failCount++;
				platform_CuStringAppendFormat(details, "%d) %s: %s" NEWLINE,
		...
		platform_CuStringAppendFormat(details, "Passes: %d ", testSuite->count - testSuite->failCount);
