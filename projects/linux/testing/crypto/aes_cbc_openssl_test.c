...
#include <string.h>
#include "testing.h"
#include "crypto/aes_cbc_openssl.h"
#include "crypto/aes_cbc_openssl_static.h"
#include "testing/crypto/aes_cbc_testing.h"


TEST_SUITE_LABEL ("aes_cbc_openssl");


/**
 * Dependencies for testing.
 */
struct aes_cbc_openssl_testing {
	struct aes_cbc_engine_openssl_state state;	/**< Variable context for the AES-CBC engine. */
	struct aes_cbc_engine_openssl test;			/**< The engine under test. */
};
...
static void aes_cbc_openssl_test_init (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;

	TEST_START;

	status = aes_cbc_openssl_init (&engine.test, &engine.state);
	...
	aes_cbc_openssl_release (&engine.test);
}

static void aes_cbc_openssl_test_init_null (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	...
	status = aes_cbc_openssl_init (NULL, &engine.state);
	CuAssertIntEquals (test, AES_CBC_ENGINE_INVALID_ARGUMENT, status);

	status = aes_cbc_openssl_init (&engine.test, NULL);
	CuAssertIntEquals (test, AES_CBC_ENGINE_INVALID_ARGUMENT, status);
}

static void aes_cbc_openssl_test_static_init (CuTest *test)
{
	struct aes_cbc_openssl_testing engine = {
		.test = aes_cbc_openssl_static_init (&engine.state)
	};
	int status;
	...
	status = aes_cbc_openssl_init_state (&engine.test);
	CuAssertIntEquals (test, 0, status);

	aes_cbc_openssl_release (&engine.test);
}

static void aes_cbc_openssl_test_static_init_null (CuTest *test)
{
	struct aes_cbc_openssl_testing engine = {
		.test = aes_cbc_openssl_static_init (NULL)
	};
	int status;

	TEST_START;

	status = aes_cbc_openssl_init_state (NULL);
	CuAssertIntEquals (test, AES_CBC_ENGINE_INVALID_ARGUMENT, status);
...
static void aes_cbc_openssl_test_release_null (CuTest *test)
{
	TEST_START;

	aes_cbc_openssl_release (NULL);
}

static void aes_cbc_openssl_test_encrypt_data_single_block (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_encrypt_data_multi_block (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_encrypt_data_same_buffer (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_encrypt_data_partial (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_LONG_DATA_LEN * 2];
...
static void aes_cbc_openssl_test_encrypt_data_partial_same_buffer (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_LONG_DATA_LEN * 2];
...
static void aes_cbc_openssl_test_encrypt_data_partial_same_iv_buffer (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_LONG_DATA_LEN * 2];
...
static void aes_cbc_openssl_test_encrypt_data_static_init (CuTest *test)
{
	struct aes_cbc_openssl_testing engine = {
		.test = aes_cbc_openssl_static_init (&engine.state)
	};
...
static void aes_cbc_openssl_test_encrypt_data_null (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_SINGLE_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_encrypt_data_small_buffer (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_MULTI_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_encrypt_data_not_block_aligned (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_MULTI_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_encrypt_data_zero_length (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_SINGLE_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_encrypt_data_no_key (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_SINGLE_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_set_key_null (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_set_key_bad_length (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_set_key_unsupported_length (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_decrypt_data_single_block (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_decrypt_data_multi_block (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_decrypt_data_same_buffer (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_decrypt_data_partial (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_LONG_DATA_LEN * 2];
...
static void aes_cbc_openssl_test_decrypt_data_partial_same_buffer (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_LONG_DATA_LEN * 2];
...
static void aes_cbc_openssl_test_decrypt_data_partial_same_iv_buffer (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_LONG_DATA_LEN * 2];
...
static void aes_cbc_openssl_test_decrypt_data_static_init (CuTest *test)
{
	struct aes_cbc_openssl_testing engine = {
		.test = aes_cbc_openssl_static_init (&engine.state)
	};
...
static void aes_cbc_openssl_test_decrypt_data_null (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_SINGLE_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_decrypt_data_small_buffer (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_MULTI_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_decrypt_data_not_block_aligned (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_MULTI_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_decrypt_data_zero_length (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_SINGLE_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_decrypt_data_no_key (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_SINGLE_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_encrypt_and_decrypt (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_encrypt_with_different_keys (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
static void aes_cbc_openssl_test_clear_key_encrypt (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t ciphertext[AES_CBC_TESTING_SINGLE_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_clear_key_decrypt (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
	uint8_t plaintext[AES_CBC_TESTING_SINGLE_BLOCK_LEN * 2];
...
static void aes_cbc_openssl_test_clear_key_static_init (CuTest *test)
{
	struct aes_cbc_openssl_testing engine = {
		.test = aes_cbc_openssl_static_init (&engine.state)
	};
...
static void aes_cbc_openssl_test_clear_key_null (CuTest *test)
{
	struct aes_cbc_openssl_testing engine;
	int status;
...
// *INDENT-OFF*
TEST_SUITE_START (aes_cbc_openssl);

TEST (aes_cbc_openssl_test_init);
TEST (aes_cbc_openssl_test_init_null);
TEST (aes_cbc_openssl_test_static_init);
TEST (aes_cbc_openssl_test_static_init_null);
TEST (aes_cbc_openssl_test_release_null);
TEST (aes_cbc_openssl_test_encrypt_data_single_block);
TEST (aes_cbc_openssl_test_encrypt_data_multi_block);
TEST (aes_cbc_openssl_test_encrypt_data_same_buffer);
TEST (aes_cbc_openssl_test_encrypt_data_partial);
TEST (aes_cbc_openssl_test_encrypt_data_partial_same_buffer);
TEST (aes_cbc_openssl_test_encrypt_data_partial_same_iv_buffer);
TEST (aes_cbc_openssl_test_encrypt_data_static_init);
TEST (aes_cbc_openssl_test_encrypt_data_null);
TEST (aes_cbc_openssl_test_encrypt_data_small_buffer);
TEST (aes_cbc_openssl_test_encrypt_data_not_block_aligned);
TEST (aes_cbc_openssl_test_encrypt_data_zero_length);
TEST (aes_cbc_openssl_test_encrypt_data_no_key);
TEST (aes_cbc_openssl_test_set_key_null);
TEST (aes_cbc_openssl_test_set_key_bad_length);
TEST (aes_cbc_openssl_test_set_key_unsupported_length);
TEST (aes_cbc_openssl_test_decrypt_data_single_block);
TEST (aes_cbc_openssl_test_decrypt_data_multi_block);
TEST (aes_cbc_openssl_test_decrypt_data_same_buffer);
TEST (aes_cbc_openssl_test_decrypt_data_partial);
TEST (aes_cbc_openssl_test_decrypt_data_partial_same_buffer);
TEST (aes_cbc_openssl_test_decrypt_data_partial_same_iv_buffer);
TEST (aes_cbc_openssl_test_decrypt_data_static_init);
TEST (aes_cbc_openssl_test_decrypt_data_null);
TEST (aes_cbc_openssl_test_decrypt_data_small_buffer);
TEST (aes_cbc_openssl_test_decrypt_data_not_block_aligned);
TEST (aes_cbc_openssl_test_decrypt_data_zero_length);
TEST (aes_cbc_openssl_test_decrypt_data_no_key);
TEST (aes_cbc_openssl_test_encrypt_and_decrypt);
TEST (aes_cbc_openssl_test_encrypt_with_different_keys);
TEST (aes_cbc_openssl_test_clear_key_encrypt);
TEST (aes_cbc_openssl_test_clear_key_decrypt);
TEST (aes_cbc_openssl_test_clear_key_static_init);
TEST (aes_cbc_openssl_test_clear_key_null);
